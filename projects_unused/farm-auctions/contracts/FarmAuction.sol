...
contract FarmAuction is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Constructor
     * @param _qubeswapToken: address of the $Qst token
     ...
     * @param _amount: amount of the bid in $Qst token
    ...
    function claimAuction(uint256 _auctionId) external nonReentrant {
        ...
        qubeswapToken.safeTransfer(address(msg.sender), claimableAmount);
    ...
    /**
     * @notice Close an auction round, and store $Qst leaderboard threshold
     * @param _bidLimit: minimal $Qst committed to be a winner
     * @dev Callable by operator
     */
    ...
    /**
     * @notice Claim $Qst for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(qubeswapToken), "Recover: Cannot be Qst token");
