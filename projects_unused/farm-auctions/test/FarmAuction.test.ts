...
const MockQst = artifacts.require("./test/MockQst");

contract("FarmAuction", ([owner, operator, alice, bob, carol, david, eve]) => {
  let mockQst, fakeQst, farmAuction;
  let result: any;

  let startBlock, endBlock;

  before(async () => {
    mockQst = await MockQst.new("QubeSwap", "Qst", parseEther("100000"), { from: owner });
    fakeQst = await MockQst.new("FakeSwap", "Qst", parseEther("100000"), { from: owner });
    farmAuction = await FarmAuction.new(mockQst.address, operator, "28800", {
      from: owner,
    });

    await mockQst.mintTokens(parseEther("100000"), { from: alice });
    await mockQst.mintTokens(parseEther("100000"), { from: bob });
    await fakeQst.mintTokens(parseEther("100000"), { from: david });
    await mockQst.approve(farmAuction.address, constants.MAX_UINT256, { from: alice });
    await mockQst.approve(farmAuction.address, constants.MAX_UINT256, { from: bob });

    await fakeQst.transfer(farmAuction.address, parseEther("100"), { from: david });
  });
  ...
  describe("Contract cannot be deployed with wrong parameters", async () => {
    it("Contract cannot be deployed with a wrong max auction length (0)", async () => {
      await expectRevert(
        FarmAuction.new(mockQst.address, operator, "0", {
          from: owner,
        }),
    ...
    it("Contract cannot be deployed with a wrong max auction length (123456789)", async () => {
      await expectRevert(
        FarmAuction.new(mockQst.address, operator, "123456789", {
          from: owner,
        }),
  ...
  describe("Admin can manage the contract", async () => {
    ...
    it("Admin can recover token", async () => {
      ...
      result = await farmAuction.recoverToken(fakeQst.address, recoveryAmount, { from: owner });

      expectEvent(result, "TokenRecovery", { token: fakeQst.address, amount: recoveryAmount.toString() });
    ...
    it("Admin cannot recover $Qst token", async () => {
      await expectRevert(
        farmAuction.recoverToken(mockQst.address, parseEther("100"), { from: owner }),
        "Recover: Cannot be Qst token"
      );
    });
  });
});
