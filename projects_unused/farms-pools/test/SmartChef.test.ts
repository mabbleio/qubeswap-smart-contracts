...
contract("Smart Chef V2", ([alice, bob, carol, david, erin, frank, ...accounts]) => {
  // Contracts
  let mockQST, mockPT, smartChef, smartChef2;
  ...
  before(async () => {
    ...
    mockQST = await MockBEP20.new("Mock QST", "QST", parseEther("1000000"), {
      from: alice,
    });
  ...
  describe("SMART CHEF #2 - POOL LIMIT", async () => {
    ...
    it("User cannot deposit more than limit", async () => {
      await mockQST.approve(smartChef2.address, parseEther("100"), {
        from: bob,
      });
    ...
    it("Admin changes the limit to 10 QST tokens", async () => {
      assert.equal(await smartChef2.hasUserLimit(), true);
    ...
    it("Admin cannot reduce the limit to 5 QST tokens", async () => {
      await expectRevert(
  ...
  describe("#3 - OTHER TESTS", async () => {
    ...
    it("30 decimals is not supported", async () => {
      ...
      await expectRevert(
        SmartChef.new(mockQST.address, mockPT2.address, rewardPerBlock, startBlock, endBlock, poolLimitPerUser),
        "Must be inferior to 30"
      );
    });
    it("Cannot deploy a pool with wrong tokens", async () => {
      await expectRevert(
        SmartChef.new(mockQST.address, mockQST.address, rewardPerBlock, startBlock, endBlock, poolLimitPerUser, {
          from: alice,
        }),
      ...
      await expectRevert(
        SmartChef.new(mockQST.address, smartChef.address, rewardPerBlock, startBlock, endBlock, poolLimitPerUser, {
          from: alice,
        }),
      ...
      await expectRevert(
        SmartChef.new(alice, mockQST.address, rewardPerBlock, startBlock, endBlock, poolLimitPerUser, { from: alice }),
