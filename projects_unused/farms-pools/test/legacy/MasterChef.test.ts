...
contract("MasterChef", ([alice, bob, dev, minter]) => {
  let qst, syrup, lp1, lp2, lp3, chef;
  ...
  beforeEach(async () => {
    ...
    chef = await MasterChef.new(qst.address, syrup.address, dev, "1000", "100", { from: minter });
    await qst.transferOwnership(chef.address, { from: minter });
  ...
  it("real case", async () => {
    ...
    assert.equal((await qst.balanceOf(alice)).toString(), "0");
    await chef.deposit(1, "20", { from: alice });
    await chef.withdraw(1, "20", { from: alice });
    assert.equal((await qst.balanceOf(alice)).toString(), "263");

    await qst.approve(chef.address, "1000", { from: alice });
    await chef.enterStaking("20", { from: alice });
    await chef.enterStaking("0", { from: alice });
    await chef.enterStaking("0", { from: alice });
    await chef.enterStaking("0", { from: alice });
    assert.equal((await qst.balanceOf(alice)).toString(), "993");
  });
  ...
  it("deposit/withdraw", async () => {
    ...
    assert.equal((await qst.balanceOf(alice)).toString(), "999");
    assert.equal((await qst.balanceOf(dev)).toString(), "100");
    ...
    assert.equal((await qst.balanceOf(bob)).toString(), "125");
  ...
  it("updaate multiplier", async () => {
    ...
    await qst.approve(chef.address, "100", { from: alice });
    await qst.approve(chef.address, "100", { from: bob });
    ...
    assert.equal((await qst.balanceOf(alice)).toString(), "700");
    assert.equal((await qst.balanceOf(bob)).toString(), "150");
