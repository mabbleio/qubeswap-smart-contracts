...
contract("IFO V2", ([alice, bob, carol, david, erin, frank, ...accounts]) => {
  ...
  // Contracts
  let mockBunnies, mockQst, mockIFO, mockOC, mockLP, qubeProfile;
  ...
  describe("IFO #1 - Initial set up", async () => {
    it("The IFO #1 is deployed and initialized", async () => {
      ...
      mockIFO = await IFOV2.new(mockLP.address, mockOC.address, qubeProfile.address, _startBlock, _endBlock, alice, {
        from: alice,
      });
  ...
  describe("IFO #1 - OVERFLOW FOR BOTH POOLS", async () => {
    ...
    it("Bob harvests for pool1", async () => {
      ...
      expectEvent.notEmitted.inTransaction(result.receipt.transactionHash, qubeProfile, "UserPointIncrease");
  ...
  describe("IFO #2 - UNDERFLOW FOR BOTH POOLS", async () => {
    ...
    it("It is not possible to set an IFO with wrong BEP20 tokens", async () => {
      await expectRevert(
        IFOV2.new(mockLP.address, mockLP.address, qubeProfile.address, _startBlock, _endBlock, alice, {
          from: alice,
        }),
        "Operations: Tokens must be be different"
      );
      await expectRevert(
        IFOV2.new(alice, mockOC.address, qubeProfile.address, _startBlock, _endBlock, alice, {
          from: alice,
        }),
      ...
      await expectRevert(
        IFOV2.new(mockLP.address, mockIFO.address, qubeProfile.address, _startBlock, _endBlock, alice, {
          from: alice,
        }),
