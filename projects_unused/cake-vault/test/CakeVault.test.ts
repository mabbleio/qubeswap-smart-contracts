...
const QstVault = artifacts.require("QstVault");
const VaultOwner = artifacts.require("VaultOwner");
const MockERC20 = artifacts.require("MockERC20");

const REWARDS_START_BLOCK = 100;

contract("QstVault", ([owner, admin, treasury, user1, user2, user3, harvester]) => {
  let vault, masterchef, qst, syrup, rewardsStartBlock;
  ...
  async function getUserInfo(user: any) {
    ...
    return {
      ...
      qstAtLastUserAction: userInfo[2],
  ...
  beforeEach(async () => {
    ...
    qst = await QubeSwapToken.new({ from: owner });
    syrup = await SyrupBar.new(qst.address, { from: owner });
    masterchef = await MasterChef.new(qst.address, syrup.address, owner, ether("1"), rewardsStartBlock, {
      from: owner,
    }); // 1 qst per block, starts at +100 block of each test
    vault = await QstVault.new(qst.address, syrup.address, masterchef.address, admin, treasury, { from: owner });

    await qst.mint(user1, ether("100"), { from: owner });
    await qst.mint(user2, ether("100"), { from: owner });
    await qst.mint(user3, ether("100"), { from: owner });
    await qst.approve(vault.address, ether("1000"), { from: user1 });
    await qst.approve(vault.address, ether("1000"), { from: user2 });
    await qst.approve(vault.address, ether("1000"), { from: user3 });
    await qst.transferOwnership(masterchef.address, { from: owner });
    await syrup.transferOwnership(masterchef.address, { from: owner });
  });

  it("Initialize", async () => {
    assert.equal(await qst.balanceOf(vault.address), 0);
    assert.equal(await qst.balanceOf(vault.address), 0);
    assert.equal(await vault.token(), qst.address);
  ...
  it("Should deposit funds, withdraw funds and get shares without rewards", async () => {
    ...
    assert.equal((await qst.balanceOf(user1)).toString(), ether("100").toString());
    assert.equal((await qst.balanceOf(user2)).toString(), ether("100").toString());
    assert.equal((await qst.balanceOf(user3)).toString(), ether("100").toString());

    await vault.deposit(ether("10"), { from: user1 });
    await vault.deposit(ether("20"), { from: user2 });
    await vault.deposit(await qst.balanceOf(user3), { from: user3 });

    // Rewards not started yet, so no rewards are earned, so all ratios should be 1:1
    // 1 share should equal to 1 qst, pricePerFullShare should equal to 1
    ...
    let user1Qst = (await getUserInfo(user1)).qstAtLastUserAction;
    let user2Qst = (await getUserInfo(user2)).qstAtLastUserAction;
    let user3Qst = (await getUserInfo(user3)).qstAtLastUserAction;
    ...
    assert.equal(user1Qst.toString(), ether("10").toString());
    assert.equal(user2Qst.toString(), ether("20").toString());
    assert.equal(user3Qst.toString(), ether("100").toString());
    ...
    assert.equal((await qst.balanceOf(user1)).toString(), ether("90").toString());
    assert.equal((await qst.balanceOf(user2)).toString(), ether("80").toString());
    assert.equal((await qst.balanceOf(user3)).toString(), ether("0").toString());
    ...
    assert.equal((await qst.balanceOf(user1)).toString(), ether("80").toString());
    assert.equal((await qst.balanceOf(user2)).toString(), ether("60").toString());
    assert.equal((await qst.balanceOf(user3)).toString(), ether("0").toString());
    ...
    assert.equal((await getUserInfo(user1)).qstAtLastUserAction, 0);
    assert.equal((await getUserInfo(user2)).qstAtLastUserAction, 0);
    assert.equal((await getUserInfo(user3)).qstAtLastUserAction, 0);
  ...
  it("Should deposit funds, withdraw funds and get shares with rewards", async () => {
    ...
    // 1 share should be > 1 qst, pricePerFullShare should equal be > 1
    let tx = await vault.deposit(ether("10"), { from: user1 }); // Vault receives 0 qst pending reward
    user1Shares = (await getUserInfo(user1)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    tx = await vault.deposit(ether("20"), { from: user2 }); // Vault receives 1 qst pending reward
    user2Shares = (await getUserInfo(user2)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    tx = await vault.deposit(await qst.balanceOf(user3), { from: user3 }); // Deposits 100 + 1 available, Vault receives 0.99999999999 qst pending reward
    user3Shares = (await getUserInfo(user3)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    // Repeated deposit
    tx = await vault.deposit(ether("10"), { from: user1 }); // Vault receives 0.999999999966 qst pending reward
    user1Shares = (await getUserInfo(user1)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    tx = await vault.deposit(ether("20"), { from: user2 }); // Vault receives 0.999999999981929578 qst pending reward
    user2Shares = (await getUserInfo(user2)).shares;
    pricePerFullShare = await vault.getPricePerFullShare();
    ...
    assert.equal(await vault.totalShares(), 0);
    assert.equal((await qst.balanceOf(user1)).toString(), ether("100.824657119370218484").toString()); // Expect > 100
    assert.equal((await qst.balanceOf(user2)).toString(), ether("101.807758814702448003").toString()); // Expect > user1Shares
    assert.equal((await qst.balanceOf(user3)).toString(), ether("105.367584065575769956").toString()); // Expect > user2Shares
  });
  ...
  it("Should withdraw with withdraw fee", async () => {
    ...
    // await vault.setWithdrawFeePeriod(time.duration.hours(9), { from: admin });
    // Withdraw fee: 0.1%
    assert.equal(await qst.balanceOf(user1), ether("100").toString());
    assert.equal(await qst.balanceOf(treasury), 0);

    // Time travel to start of rewards
    await time.advanceBlockTo(rewardsStartBlock);

    await vault.deposit(ether("10"), { from: user1 }); // lastDepositedAt starts
    assert.equal(await qst.balanceOf(user1), ether("90").toString());
    assert.equal((await vault.available()).toString(), ether("0").toString());
    assert.equal((await vault.balanceOf()).toString(), ether("10").toString());
    let balance = await qst.balanceOf(user1);
    ...
    assert.equal((await qst.balanceOf(user1)).toString(), balance.add(amountForShares).sub(withdrawFee).toString());
    assert.equal((await qst.balanceOf(treasury)).toString(), withdrawFee.toString());
    balance = await qst.balanceOf(user1);
    ...
    assert.equal((await qst.balanceOf(user1)).toString(), balance.add(amountForShares).toString());
    assert.equal((await qst.balanceOf(treasury)).toString(), withdrawFee.toString()); // No change
  });
  ...
  it("Should harvest and reinvest funds", async () => {
    ...
    await vault.deposit(ether("10"), { from: user1 }); // Vault receives 0 qst pending reward
    await vault.deposit(ether("20"), { from: user2 }); // Vault receives 1 qst pending reward

    assert.equal((await vault.available()).toString(), ether("1").toString());
    assert.equal((await vault.balanceOf()).toString(), ether("31").toString());

    let pendingQst = ether("0.99999999999");
    await vault.harvest({ from: harvester }); // Receives 0.99999999999 pending qst reward

    assert.equal((await vault.available()).toString(), 0);
    assert.equal((await vault.balanceOf()).toString(), ether("31").add(pendingQst).toString());
    const balance = await vault.balanceOf();

    pendingQst = ether("0.9999999999996875");
    const tx = await vault.harvest({ from: harvester }); // Receives 0.9999999999996875 pending qst reward
    ...
    assert.equal((await vault.available()).toString(), 0);
    assert.equal((await vault.balanceOf()).toString(), balance.add(pendingQst).toString());
  });

  it("Should harvest with performance and call fees", async () => {
    // Performance fee: 2%, Call fee: 0.25%
    assert.equal(await qst.balanceOf(treasury), 0);
    assert.equal(await qst.balanceOf(harvester), 0);
    ...
    let tx = await vault.harvest({ from: harvester }); // Receives 0.99999999999 pending qst reward
    ...
    assert.equal((await qst.balanceOf(treasury)).toString(), treasuryFee.toString());
    assert.equal((await qst.balanceOf(harvester)).toString(), harvesterFee.toString());
    assert.equal(await vault.available(), 0);
    assert.equal(
      (await vault.balanceOf()).toString(),
      ether("31").add(pending).sub(treasuryFee).sub(harvesterFee).toString()
    );
    let treasuryTotal = await qst.balanceOf(treasury);
    let harvesterTotal = await qst.balanceOf(harvester);
    ...
    tx = await vault.harvest({ from: harvester }); // Receives 0.99999999999 pending qst reward
    ...
    assert.equal((await qst.balanceOf(treasury)).toString(), treasuryTotal.add(treasuryFee).toString());
    assert.equal((await qst.balanceOf(harvester)).toString(), harvesterTotal.add(harvesterFee).toString());
    assert.equal(await vault.available(), 0);
    assert.equal(
      (await vault.balanceOf()).toString(),
  ...
  it("Should withdraw non-qst and non-syrup tokens inCaseTokensGetStuck", async () => {
    await vault.deposit(ether("10"), { from: user1 });

    await expectRevert(vault.inCaseTokensGetStuck(qst.address), "admin: wut?");
    await expectRevert(vault.inCaseTokensGetStuck(syrup.address), "admin: wut?");
    await expectRevert(
      vault.inCaseTokensGetStuck(qst.address, { from: admin }),
    ...
    // Send non-qst/non-syrup tokens to vault (supposedly by mistake)
    const mockQst = await QubeSwapToken.new({ from: owner });
    await mockQst.mint(user1, ether("100"), { from: owner });
    await mockQst.transfer(vault.address, ether("99"), { from: user1 });
    assert.equal((await mockQst.balanceOf(vault.address)).toString(), ether("99").toString()); // Vault has 99 mockQst

    await vault.inCaseTokensGetStuck(mockQst.address, { from: admin });
    assert.equal((await mockQst.balanceOf(vault.address)).toString(), ether("0").toString()); // Vault has 0 mockQst
    assert.equal((await mockQst.balanceOf(admin)).toString(), ether("99").toString()); // Admin now has 99 mockQst
  });
  ...
  it("Should disallow deposits and harvest when paused", async () => {
    ...
    await vault.deposit(ether("10"), { from: user1 });
    await vault.deposit(await qst.balanceOf(user1), { from: user1 });
    await vault.harvest();
  });
