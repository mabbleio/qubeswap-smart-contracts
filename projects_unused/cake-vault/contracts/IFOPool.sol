...
contract IFOPool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 qstAtLastUserAction; // keeps track of qst deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }
    //IFO
    struct UserIFOInfo {
        // ifo valid period is current block between startblock and endblock
        uint256 lastActionBalance; // staked qst numbers (not include compoud qst) at last action
        uint256 lastValidActionBalance; // staked qst numbers in ifo valid period
    ...
    IERC20 public immutable token; // Qst token
    ...
    /**
     * @notice Deposits funds into the Qst Vault
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.qstAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
     ...
     * @param _lastValidActionBalance: staked qst number at last action.
     ...
     * @param _amount:the qst amount that need be add or sub
    ...
    /**
     * @notice original Withdraws implementation from funds, the logic same as Qst Vault withdraw
    ...
    /**
     * @notice Reinvests QST tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
     ...
     * @return Expected reward to collect in QST
     */
    function calculateHarvestQstRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingQst(0, address(this));
     ...
     * @return Returns total pending qst rewards
     */
    function calculateTotalPendingQstRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingQst(0, address(this));
