...
const main = async () => {
  ...
  if (name == "mainnet") {
    ...
    if (!config.Qst[name] || config.Qst[name] === constants.ZERO_ADDRESS) {
      throw new Error("Missing syrup address, refer to README 'Deployment' section");
    }
  ...
  let qst, syrup, masterchef, admin, treasury;
  ...
  if (name == "mainnet") {
    ...
    const QstContract = await ethers.getContractFactory("QubeSwapToken");
    ...
    qst = (await QstContract.deploy()).address;
    await qst.deployed();
    syrup = (await SyrupContract.deploy(qst)).address;
    await syrup.deployed();
    masterchef = (await MasterChefContract.deploy(qst, syrup, admin, ethers.BigNumber.from("1"), currentBlock))
      .address;
    ...
    console.log("Qst deployed to:", qst);
    console.log("Syrup deployed to:", syrup);
    console.log("MasterChef deployed to:", masterchef);
  }

  console.log("Deploying Qst Vault...");

  const QstVaultContract = await ethers.getContractFactory("QstVault");
  const qstVault = await QstVaultContract.deploy(qst, syrup, masterchef, admin, treasury);
  await qstVault.deployed();

  console.log("QstVault deployed to:", qstVault.address);
};
