...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when QST mining ends.
    ...
    // Qube Profile is requested
    bool public qubeProfileIsRequested;

    // Qube Profile points threshold
    uint256 public qubeProfileThresholdPoints;
     ...
     * @param _qubeProfileIsRequested: Qube Profile is requested
     * @param _qubeProfileThresholdPoints: Qube Profile need threshold points
     */
    constructor(
        address _qubeProfile,
        bool _qubeProfileIsRequested,
        uint256 _qubeProfileThresholdPoints
    ) {
        ...
        // if qubeProfile is requested
        qubeProfileIsRequested = _qubeProfileIsRequested;

        // qubeProfile threshold points when profile & points are requested
        qubeProfileThresholdPoints = _qubeProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!qubeProfileIsRequested && qubeProfileThresholdPoints == 0) ||
                qubeProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (qubeProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = qubeProfile.getUserProfile(msg.sender);
        }

        require(
            qubeProfileThresholdPoints == 0 || numberUserPoints >= qubeProfileThresholdPoints,
        ...
        onlyOwner
    {
        ...
        qubeProfileIsRequested = _isRequested;
        qubeProfileThresholdPoints = _thresholdPoints;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 qstReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare + (qstReward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 qstReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (qstReward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardBlock = block.number;
    }
