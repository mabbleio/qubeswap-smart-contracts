...
import {IQubePair} from "./interfaces/IQubePair.sol";
import {IQubeRouter02} from "./interfaces/IQubeRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by QubeSwap teams.
 */
contract QubeZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // QubeRouter interface
    IQubeRouter02 public qubeRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from QubeSwap)
    ...
    // Address QubeRouter
    address private qubeRouterAddress;
     ...
     * @param _qubeRouter: address of the QubeRouter
    ...
    constructor(
        ...
        qubeRouterAddress = _qubeRouter;
        qubeRouter = IQubeRouter02(_qubeRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. QST/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. QST) to receive in the intermediary swap (e.g. BNB --> QST)
     */
     ...
     * @param _lpToken: LP token address (e.g. QST/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. QST) in the intermediary swap (e.g. BUSD --> QST)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. QST/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. QST --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. QST/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. QST or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. QST) in the intermediary swap (e.g. BUSD --> QST)
     */
    ...
    {
        address token0 = IQubePair(_lpToken).token0();
        address token1 = IQubePair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IQubePair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = qubeRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = qubeRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IQubePair(_lpToken).token0() || _token0ToZap == IQubePair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IQubePair(_lpToken).token0() || _token1ToZap == IQubePair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IQubePair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IQubePair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IQubePair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = qubeRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = qubeRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IQubePair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "QubeRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "QubeRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        qubeRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), qubeRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(qubeRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = qubeRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = qubeRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = qubeRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = qubeRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = qubeRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
