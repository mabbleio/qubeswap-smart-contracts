...
contract("IFO Point logic", ([alice, bob, carol, david, erin, frank]) => {
  ...
  let mockQst, mockBunnies, mockIFO, mockLP, mockOC, qubeProfile, pointCenterIFO;
  ...
  before(async () => {
    ...
    pointCenterIFO = await PointCenterIFO.new(qubeProfile.address, "10", {
      from: alice,
    });
  ...
  describe("Initial contract parameters for all contracts", async () => {
    ...
    it("MockQST is correct", async () => {
      ...
      assert.equal(String(await mockQst.balanceOf(alice)), String(parseEther("50")));
      assert.equal(String(await mockQst.totalSupply()), String(parseEther("50")));
    });
  ...
  describe("Initial set up", async () => {
    ...
    it("Bob/Carol/David/Erin create a profile in the system", async () => {
      ...
      for (let thisUser of [bob, carol, david, erin]) {
        // Mints 100 QST
        await mockQst.mintTokens("100000000000000000000", { from: thisUser });
        ...
        // Bob approves QST to be spent by QubeProfile
    ...
    it("Bob deactivates his profile and tries to get points", async () => {
      result = await qubeProfile.pauseProfile({ from: bob });
    ...
    it("Alice makes this application a pointAdmin", async () => {
      result = await qubeProfile.grantRole(POINT_ROLE, pointCenterIFO.address, {
        from: alice,
      });
