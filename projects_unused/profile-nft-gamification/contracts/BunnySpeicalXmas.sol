...
contract BunnySpecialXmas is Ownable {
    ...
    event NewQubeProfileThresholdPoints(uint256 thresholdPoints);
     ...
     * @param _qubeProfileThresholdPoints: User points threshold for mint NFT
    ...
    function updateThresholdPoints(uint256 _newThresholdPoints) external onlyOwner {
        qubeProfileThresholdPoints = _newThresholdPoints;
        emit NewQubeProfileThresholdPoints(_newThresholdPoints);
    }
    ...
    function canClaim(address _userAddress) public view returns (bool) {
        (, uint256 numberUserPoints, , , , bool active) = qubeProfile.getUserProfile(_userAddress);
        // If user is able to mint this NFT
        if (
            !hasClaimed[_userAddress] &&
            block.number < endBlock &&
            active &&
            numberUserPoints >= qubeProfileThresholdPoints
        ) {
            return true;
        }
        return false;
