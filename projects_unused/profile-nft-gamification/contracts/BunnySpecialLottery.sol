...
import {IQubeSwapLottery} from "lottery/contracts/interfaces/IQubeSwapLottery.sol";
...
contract BunnySpecialLottery is Ownable {
    ...
    IQubeSwapLottery public qubeSwapLottery;
    ...
    constructor(
        address _qubeSwapLotteryAddress,
        ...
        qubeSwapLottery = IQubeSwapLottery(_qubeSwapLotteryAddress);
        bunnyMintingStation = BunnyMintingStation(_bunnyMintingStationAddress);
        qubeProfile = QubeProfile(_qubeProfileAddress);
    ...
    function mintNFT(
        ...
        // Increase point on QubeSwap profile, for a given campaignId.
        qubeProfile.increaseUserPoints(msg.sender, numberPoints[_bunnyId], campaignIds[_bunnyId]);
    ...
    /**
     * @notice Change the campaignId for QubeSwap Profile.
     * @dev Only callable by owner.
     */
    ...
    /**
     * @notice Change the number of points for QubeSwap Profile.
     * @dev Only callable by owner.
     */
    ...
    function _canClaim(
        ...
        if (
            ...
            !qubeProfile.getUserStatus(_userAddress) ||
        ...
        if (_bunnyId == nftId1) {
            uint256 size;
            (, , , size) = qubeSwapLottery.viewUserInfoForLotteryId(_userAddress, _lotteryId, 0, 1);
            return size > 0;
        }
        if (_bunnyId == nftId2) {
            ...
            (, , ticketStatuses, size) = qubeSwapLottery.viewUserInfoForLotteryId(
                _userAddress,
                _lotteryId,
                _cursor,
                1
