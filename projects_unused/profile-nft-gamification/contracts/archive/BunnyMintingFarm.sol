...
contract BunnyMintingFarm is Ownable {
    ...
    QubeBunnies public qubeBunnies;
    IBEP20 public qubeswapToken;
    ...
    // end block number to claim QSTs by burning NFT
    uint256 public endBlockNumber;

    // number of total bunnies burnt
    uint256 public countBunniesBurnt;

    // Number of QSTs a user can collect by burning her NFT
    uint256 public qstPerBurn;
    ...
    constructor(
        ...
        qubeBunnies = new QubeBunnies(_baseURI);
        qubeswapToken = _qubeswapToken;
        totalSupplyDistributed = _totalSupplyDistributed;
        qstPerBurn = _qstPerBurn;
        baseURI = _baseURI;
        ...
        qubeBunnies.setBunnyName(0, "Swapsies");
        qubeBunnies.setBunnyName(1, "Drizzle");
        qubeBunnies.setBunnyName(2, "Blueberries");
        qubeBunnies.setBunnyName(3, "Circular");
        qubeBunnies.setBunnyName(4, "Sparkle");
    }

    /**
     * @dev Mint NFTs from the QubeBunnies contract.
     * Users can specify what bunnyId they want to mint. Users can claim once.
     * There is a limit on how many are distributed. It requires QST balance to be >0.
     */
    function mintNFT(uint8 _bunnyId) external {
        ...
        // Check whether user owns any QST
        require(qubeswapToken.balanceOf(msg.sender) > 0, "Must own QST");
        ...
        uint256 tokenId = qubeBunnies.mint(address(msg.sender), tokenURI, _bunnyId);

        emit BunnyMint(msg.sender, tokenId, _bunnyId);
    }

    /**
     * @dev Burn NFT from the QubeBunnies contract.
     * Users can burn their NFT to get a set number of QST.
     * There is a cap on how many can be distributed for free.
     */
    function burnNFT(uint256 _tokenId) external {
        require(qubeBunnies.ownerOf(_tokenId) == msg.sender, "Not the owner");
        require(block.number < endBlockNumber, "too late");

        qubeBunnies.burn(_tokenId);
        countBunniesBurnt = countBunniesBurnt.add(1);
        qubeswapToken.safeTransfer(address(msg.sender), qstPerBurn);
    ...
    /**
     * @dev It transfers the QST tokens back to the chef address.
     * Only callable by the owner.
     */
    function withdrawQst(uint256 _amount) external onlyOwner {
        require(block.number >= endBlockNumber, "too early");
        qubeswapToken.safeTransfer(address(msg.sender), _amount);
    }
    ...
    function changeOwnershipNFTContract(address _newOwner) external onlyOwner {
        qubeBunnies.transferOwnership(_newOwner);
    }
}
