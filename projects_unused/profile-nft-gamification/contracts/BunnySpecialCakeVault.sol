...
import "qube-qst-vault/contracts/QstVault.sol";

import "./BunnyMintingStation.sol";
import "./QubeProfile.sol";

/**
 * @title BunnySpecialQstVault.
 * @notice It is a contract for users to mint Qst Vault collectible.
 */
contract BunnySpecialQstVault is Ownable {
    using SafeMath for uint256;

    BunnyMintingStation public bunnyMintingStation;
    QstVault public qstVault;
    ...
    function mintNFT() external {
        ...
        bool isUserActive;
        (, , , , , isUserActive) = qubeProfile.getUserProfile(msg.sender);
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress]) {
            ...
            if (!qubeProfile.getUserStatus(_userAddress)) {
                return false;
            } else {
                uint256 lastDepositedTime;
                (, lastDepositedTime, , ) = qstVault.userInfo(_userAddress);
