...
import "./interfaces/IQubeProfile.sol";
...
contract TradingCompV1 is Ownable {
    ...
    IQubeProfile public qubeProfile;
    ...
    struct CompetitionRewards {
        ...
        uint256[5] qstRewards; // qst rewards per group
     ...
     * @param _qubeProfileAddress: QubeProfile address
     * @param _bunnyStationAddress: BunnyMintingStation address
     * @param _qubeswapTokenAddress: the address of the QST token
     */
    constructor(
        address _qubeProfileAddress,
        address _bunnyStationAddress,
        address _qubeswapTokenAddress
    ) public {
        qubeProfile = IQubeProfile(_qubeProfileAddress);
        bunnyMintingStation = BunnyMintingStation(_bunnyStationAddress);
        qubeswapToken = IBEP20(_qubeswapTokenAddress);
    ...
    function claimReward() external {
        ...
        if (userRewardGroup > 0) {
            qubeswapToken.safeTransfer(senderAddress, userRewards.qstRewards[userRewardGroup]);
        ...
        qubeProfile.increaseUserPoints(
            senderAddress,
     ...
     * @dev Only callable if the user has an active QubeProfile.
     */
    function register() external {
        ...
        (, , userTeamId, , , isUserActive) = qubeProfile.getUserProfile(senderAddress);
    ...
    /**
     * @notice It allows the owner to claim the QST remainder
     * @dev Only callable by owner.
     * @param _amount: amount of QST to withdraw (decimals = 18)
     */
    function claimRemainder(uint256 _amount) external onlyOwner {
        require(currentStatus == CompetitionStatus.Over, "NOT_OVER");
        qubeswapToken.safeTransfer(_msgSender(), _amount);
    }
     ...
     * @param _qstRewards: QST rewards for each user group for teamId
    ...
    function updateTeamRewards(
        ...
        _rewardCompetitions[_teamId].qstRewards = _qstRewards;
     ...
     * @return userQstRewards: the QST to claim/claimed
        ...
        )
    {
        ...
        if ((currentStatus != CompetitionStatus.Claiming) && (currentStatus != CompetitionStatus.Over)) {
            ...
            uint256 userQstRewards = _rewardCompetitions[userTeamId].qstRewards[userRewardGroup];
            ...
            return (hasUserRegistered, hasUserClaimed, userRewardGroup, userQstRewards, userPointRewards, canClaimNFT);
        }
    }
