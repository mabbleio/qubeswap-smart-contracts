...
contract BunnySpecialV1 is Ownable {
    ...
    event BunnyAdd(uint8 indexed bunnyId, uint256 thresholdUser, uint256 costQst);

    // Event to notify one of the bunnies' requirements to mint differ
    event BunnyChange(uint8 indexed bunnyId, uint256 thresholdUser, uint256 costQst, bool isActive);
    ...
    function mintNFT(uint8 _bunnyId) external {
        ...
        (userId, , , , , isUserActive) = qubeProfile.getUserProfile(senderAddress);
        ...
        if (bunnyCharacteristics[_bunnyId].qstCost > 0) {
            qubeswapToken.safeTransferFrom(senderAddress, address(this), bunnyCharacteristics[_bunnyId].qstCost);
        }
    ...
    function addBunny(
        ...
        emit BunnyAdd(_bunnyId, _thresholdUser, _qstCost);
    }
    ...
    function updateBunny(
        ...
        bunnyCharacteristics[_bunnyId].qstCost = _qstCost;
        bunnyCharacteristics[_bunnyId].isActive = _isActive;

        emit BunnyChange(_bunnyId, _thresholdUser, _qstCost, _isActive);
    }
    ...
    function canClaimSingle(address _userAddress, uint8 _bunnyId) external view returns (bool) {
        if (!qubeProfile.hasRegistered(_userAddress)) {
            return false;
        } else {
            uint256 userId;
            bool userStatus;

            (userId, , , , , userStatus) = qubeProfile.getUserProfile(_userAddress);
