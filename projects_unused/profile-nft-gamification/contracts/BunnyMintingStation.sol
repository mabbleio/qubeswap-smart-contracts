...
contract BunnyMintingStation is AccessControl {
    ...
    constructor(QubeBunnies _qubeBunnies) public {
        qubeBunnies = _qubeBunnies;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @notice Mint NFTs from the QubeBunnies contract.
    ...
    function mintCollectible(
        ...
        uint256 tokenId = qubeBunnies.mint(_tokenReceiver, _tokenURI, _bunnyId);
        return tokenId;
    }
    ...
    function setBunnyName(uint8 _bunnyId, string calldata _bunnyName) external onlyOwner {
        qubeBunnies.setBunnyName(_bunnyId, _bunnyName);
    }
