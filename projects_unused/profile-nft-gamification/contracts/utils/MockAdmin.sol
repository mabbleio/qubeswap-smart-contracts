...
contract MockAdmin is Ownable {
    QubeProfile qubeProfile;
    address public qubeProfileAddress;
    ...
    constructor(address _qubeProfileAddress) public {
        qubeProfileAddress = _qubeProfileAddress;
        qubeProfile = QubeProfile(qubeProfileAddress);
        numberFreePoints = 88;
    ...
    function increaseTeamPointsPP(uint16 _teamId, uint256 _numberPoints) external onlyOwner {
        qubeProfile.increaseTeamPoints(_teamId, _numberPoints, campaignId3);
    }
    ...
    function increaseUserPointsPP() external {
        ...
        isActive = qubeProfile.getUserStatus(_msgSender());

        require(isActive, "not active");

        // Increase the number of points
        hasReceivedPoints[_msgSender()] = true;
        qubeProfile.increaseUserPoints(_msgSender(), numberFreePoints, campaignId1);
    }
    ...
    function increaseUserPointsMultiplePP(address[] calldata _userAddresses, uint256 _numberPoints) external onlyOwner {
        qubeProfile.increaseUserPointsMultiple(_userAddresses, _numberPoints, campaignId2);
    }
}
