...
import {IQubeBunnies} from "./interfaces/IQubeBunnies.sol";

contract QubeBunniesWhitelistChecker is Ownable, ICollectionWhitelistChecker {
    IQubeBunnies public qubeBunnies;
    ...
    /**
     * @notice Constructor
     * @param _qubeBunniesAddress: QubeBunnies contract
     */
    constructor(address _qubeBunniesAddress) {
        qubeBunnies = IQubeBunnies(_qubeBunniesAddress);
    }
    ...
    function canList(uint256 _tokenId) external view override returns (bool) {
        uint8 bunnyId = qubeBunnies.getBunnyId(_tokenId);
