...
const QubeSquad = artifacts.require("QubeSquad");
...
contract("Qube Squad", ([owner, operator, alice, bob, carol, david, erin]) => {
  let mockQst, fakeQst, mockQubeBunnies, nftSale, qubeProfile, qubeSquad;
  let maxSupply;
  ...
  before(async () => {
    ...
    // Token setup
    mockQst = await MockERC20.new("QubeSwap", "Qst", amount, { from: owner });
    fakeQst = await MockERC20.new("FakeSwap", "Fake", amount, { from: owner });
    mockLink = await MockERC20.new("MockLink", "LINK", amount, { from: owner });

    // Qube Bunnies / Profile setup
    mockQubeBunnies = await MockERC721.new("Qube Bunnies", "PB", { from: owner });
    qubeProfile = await QubeProfile.new(mockQst.address, parseEther("2"), parseEther("1"), parseEther("2"), {
      from: owner,
    });

    await qubeProfile.addTeam("1st Team", "Be a Chef!", { from: owner });
    await qubeProfile.addNftAddress(mockQubeBunnies.address, { from: owner });

    maxSupply = "100";
    maxReserveSupply = "10";
    pricePerTicket = parseEther("1").toString(); // 1 QST

    // Deploy QubeSquad
    qubeSquad = await QubeSquad.new("Qube Squad", "PSQ", maxSupply, {
      from: owner,
    });
    ...
    // Handle profile creation, mint Qst, and all approvals for 4 users
    let i = 0;

    for (let user of [alice, bob, carol, david]) {
      await mockQubeBunnies.mint({ from: user });
      await mockQubeBunnies.setApprovalForAll(qubeProfile.address, true, { from: user });
      await mockQst.mintTokens(amount, { from: user });
      await mockQst.approve(qubeProfile.address, constants.MAX_UINT256, { from: user });
      await mockQst.approve(nftSale.address, constants.MAX_UINT256, { from: user });
      await qubeProfile.createProfile("1", mockQubeBunnies.address, i.toString(), { from: user });
      i++;
    }
    ...
    // Transfer owner of the QubeSquad
    await qubeSquad.transferOwnership(nftSale.address, { from: owner });
    ...
    // QST Token
    await mockQst.approve(nftSale.address, constants.MAX_UINT256, { from: alice });
    await mockQst.approve(nftSale.address, constants.MAX_UINT256, { from: bob });
  });
  ...
  describe("#1 - Normal behavior", async () => {
    ...
    it("Cannot change ownership outside of the claim phase", async () => {
      await expectRevert(nftSale.changeOwnershipQubeSquad(owner, { from: owner }), "Status: Must be in claim");
    });
    ...
    it("Alice can mint a NFT", async () => {
      ...
      expectEvent.inTransaction(result.receipt.transactionHash, qubeSquad, "Transfer", {
    ...
    it("Alice mints all her tickets", async () => {
      ...
      assert.equal(String(await qubeSquad.totalSupply()), "41");
      assert.equal(String(await qubeSquad.balanceOf(alice)), "41");
    });
    ...
    it("Bob mints all her tickets", async () => {
      ...
      assert.equal(String(await qubeSquad.ownerOf("83")), bob);
      assert.equal(String(await qubeSquad.ownerOf("0")), bob);
      assert.equal(String(await qubeSquad.ownerOf("22")), bob);

      assert.equal(String(await qubeSquad.totalSupply()), "83");
      assert.equal(String(await qubeSquad.balanceOf(bob)), "42");
    });
    ...
    it("Carol mints her tickets", async () => {
      ...
      assert.equal(String(await qubeSquad.totalSupply()), "90");
      assert.equal(String(await qubeSquad.balanceOf(carol)), "7");

      assert.equal(String(await qubeSquad.ownerOf("4")), carol);
      assert.equal(String(await qubeSquad.ownerOf("10")), carol);
    });

    it("Cannot change ownership unless all tickets are used to mint", async () => {
      await expectRevert(
        nftSale.changeOwnershipQubeSquad(owner, { from: owner }),
    ...
    it("Operator mints his tickets", async () => {
      ...
      assert.equal(String(await qubeSquad.totalSupply()), "100");
      assert.equal(String(await qubeSquad.balanceOf(operator)), "10");
      assert.equal(String(await qubeSquad.ownerOf("11")), operator);
      assert.equal(String(await qubeSquad.ownerOf("20")), operator);

      let tokens = await qubeSquad.tokensOfOwnerBySize(operator, "0", "5");
      ...
      tokens = await qubeSquad.tokensOfOwnerBySize(operator, "5", "10");
    ...
    it("Token URI works as expected", async () => {
      await expectRevert(qubeSquad.tokenURI("100"), "ERC721Metadata: URI query for nonexistent token");
      assert.equal(await qubeSquad.tokenURI("11"), mockURI + "11.json");
      assert.equal(await qubeSquad.tokenURI("0"), mockURI + "0.json");
      assert.equal(await qubeSquad.tokenURI("99"), mockURI + "99.json");
    });
    ...
    it("Owner can lock the contract", async () => {
      ...
      expectEvent.inTransaction(result.receipt.transactionHash, qubeSquad, "Lock");
    });

    it("Owner can transfer the ownership of QubeSquad", async () => {
      const result = await nftSale.changeOwnershipQubeSquad(owner, { from: owner });

      expectEvent.inTransaction(result.receipt.transactionHash, qubeSquad, "OwnershipTransferred", {
  ...
  describe("#2 - Functions revert as expected", async () => {
    it("Owner cannot lock twice", async () => {
      await expectRevert(qubeSquad.lock({ from: owner }), "Operations: Contract is locked");
    });

    it("Owner cannot set baseURI if contract is locked", async () => {
      await expectRevert(qubeSquad.setBaseURI("ipfs://wrong/", { from: owner }), "Operations: Contract is locked");
    });

    it("Owner cannot mint if token supply is reached", async () => {
      await expectRevert(qubeSquad.mint(owner, "1", { from: owner }), "NFT: Total supply reached");
      await expectRevert(qubeSquad.mint(owner, "100", { from: owner }), "NFT: Total supply reached");
    });
  ...
  describe("#3 - Owner/Operator functions", async () => {
    ...
    it("Owner can recover token from NFTSale", async () => {
      // Fake QST Token
      await fakeQst.mintTokens(parseEther("1000"), { from: david });
      await fakeQst.transfer(nftSale.address, parseEther("100"), { from: david });

      const result = await nftSale.recoverToken(fakeQst.address, { from: owner });
    ...
    it("Owner cannot recover token if balance is zero from NFTSale", async () => {
      await expectRevert(
        nftSale.recoverToken(fakeQst.address, { from: owner }),
        "Operations: Cannot recover zero balance"
      );
    });

    it("Owner cannot recover QST tokens from NFTSale", async () => {
      await expectRevert(nftSale.recoverToken(mockQst.address, { from: owner }), "Operations: Cannot recover QST");
    });

    it("Owner can recover token from QubeSquad", async () => {
      await fakeQst.transfer(qubeSquad.address, parseEther("100"), { from: david });

      const result = await qubeSquad.recoverToken(fakeQst.address, { from: owner });
      ...
      expectEvent.inTransaction(result.receipt.transactionHash, fakeQst, "Transfer", {
        from: qubeSquad.address,
        to: owner,
        value: recoveryAmount.toString(),
      });
    });

    it("Owner cannot recover token if balance is zero from QubeSquad", async () => {
      await expectRevert(
        qubeSquad.recoverToken(fakeQst.address, { from: owner }),
        "Operations: Cannot recover zero balance"
      );
    });
    ...
    it("Owner functions can only be called by the owner for NFTSale", async () => {
      await expectRevert(
        qubeSquad.recoverToken(fakeQst.address, { from: alice }),
        "Ownable: caller is not the owner"
      );

      await expectRevert(qubeSquad.lock({ from: alice }), "Ownable: caller is not the owner");

      await expectRevert(qubeSquad.mint(alice, "100", { from: alice }), "Ownable: caller is not the owner");

      await expectRevert(qubeSquad.setBaseURI("ipfs://alice", { from: alice }), "Ownable: caller is not the owner");
    });

    it("Owner functions can only be called by the owner for NFTSale", async () => {
      for (let user of [alice, operator]) {
        await expectRevert(
          nftSale.changeOwnershipQubeSquad(alice, { from: user }),
          "Ownable: caller is not the owner"
        );
        ...
        await expectRevert(nftSale.recoverToken(fakeQst.address, { from: user }), "Ownable: caller is not the owner");
      }
    });
