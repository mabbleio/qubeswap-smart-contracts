...
const QubeSwapLottery = artifacts.require("./QubeSwapLottery.sol");
...
contract("Lottery V2", ([alice, bob, carol, david, erin, operator, treasury, injector]) => {
  ...
  let _priceTicketInQst = parseEther("0.5");
  ...
  // Contracts
  let lottery, mockQst, randomNumberGenerator;
  ...
  before(async () => {
    ...
    // Deploy QubeSwapLottery
    lottery = await QubeSwapLottery.new(mockQst.address, randomNumberGenerator.address, { from: alice });
  ...
  describe("LOTTERY #1 - CUSTOM RANDOMNESS", async () => {
    ...
    it("Users mint and approve QST to be used in the lottery", async () => {
      for (let thisUser of [alice, bob, carol, david, erin, injector]) {
        await mockQst.mintTokens(parseEther("100000"), { from: thisUser });
        await mockQst.approve(lottery.address, parseEther("100000"), {
          from: thisUser,
        });
      }
    });
    ...
    it("Operator starts lottery", async () => {
      ...
      expectEvent(result, "LotteryOpen", {
        ...
        priceTicketInQst: _priceTicketInQst.toString(),
        firstTicketId: "0",
        injectedAmount: "0",
      });
    ...
    it("Owner does 10k QST injection", async () => {
    ...
    it("David claims the jackpot", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 QST collected
      // 10053.0025 * (1-0.20) * 0.5 = 4021.201 QST
    ...
    it("Bob claims 10 winning tickets he bought", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 QST collected
      // 10053.0025 * (1-0.20) * 0.02 = 160.84804 QST
      // 10053.0025 * (1-0.20) * 0.03 = 241.27206 QST
    ...
    describe("LOTTERY #2 - CUSTOM RANDOMNESS - Exceptions", async () => {
      ...
      it("Operator cannot start lottery if length is too short/long", async () => {
        ...
        await expectRevert(
          lottery.startLottery(endTimeTarget, _priceTicketInQst, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
      ...
      it("Operator cannot start lottery if discount divisor is too low", async () => {
        ...
        await expectRevert(
          lottery.startLottery(endTime, _priceTicketInQst, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
      ...
      it("Operator cannot start lottery if ticket price too low or too high", async () => {
        let newPriceTicketInQst = parseEther("0.0049999999");

        await expectRevert(
          lottery.startLottery(endTime, newPriceTicketInQst, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
          "Outside of limits"
        );

        newPriceTicketInQst = parseEther("0.0049999999");
      ...
      it("Operator cannot start a second lottery", async () => {
        await expectRevert(
          lottery.startLottery(_lengthLottery, _priceTicketInQst, _discountDivisor, _rewardsBreakdown, _treasuryFee, {
            from: operator,
          }),
    ...
    describe("Role exceptions", async () => {
      it("Owner can recover funds only if not QST token", async () => {
        // Deploy Random Token
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockQst.address, parseEther("1"), { from: alice }),
          "Cannot be QST token"
        );
      });
      ...
      it("Only owner/injector can call owner functions", async () => {
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockQst.address, parseEther("10"), { from: operator }),
          "Ownable: caller is not the owner"
        );
