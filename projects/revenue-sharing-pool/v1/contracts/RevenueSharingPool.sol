...
contract RevenueSharingPool is Ownable, ReentrancyGuard {
    ...
    /// @dev VQst supply at week bounds
    ...
    /// @notice Get VQst balance of "_user" at "_timstamp"
    ...
    function balanceOfAt(address _user, uint256 _timestamp) external view returns (uint256) {
        uint256 _maxUserEpoch = IVQst(VQst).userPointEpoch(_user);
        if (_maxUserEpoch == 0) {
            return 0;
        }

        uint256 _epoch = _findTimestampUserEpoch(_user, _timestamp, _maxUserEpoch);
        Point memory _point = IVQst(VQst).userPointHistory(_user, _epoch);
    ...
    function _checkpointTotalSupply() internal {
        ...
        for (uint256 _i = 0; _i < 52; _i++) {
            if (_weekCursor > _roundedTimestamp) {
                ...
                Point memory _point = IVQst(VQst).pointHistory(_epoch);
                int128 _timeDelta = 0;
    ...
    /// @notice Update VQst total supply checkpoint
    ...
    function _claim(address _user, uint256 _maxClaimTimestamp) internal returns (uint256) {
        ...
        Point memory _userPoint = IVQst(VQst).userPointHistory(_user, _userEpoch);
        ...
        for (uint256 _i = 0; _i < 52; _i++) {
            ...
            if (_userWeekCursor >= _userPoint.timestamp && _userEpoch <= _maxUserEpoch) {
                ...
                if (_userEpoch > _maxUserEpoch) {
                    ...
                    _userPoint = IVQst(VQst).userPointHistory(_user, _userEpoch);
                }
            } else {
    ...
    function _findTimestampEpoch(uint256 _timestamp) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < 128; i++) {
            ...
            Point memory _point = IVQst(VQst).pointHistory(_mid);
    ...
    function _findTimestampUserEpoch(
        ...
        for (uint256 i = 0; i < 128; i++) {
            ...
            uint256 _mid = (_min + _max + 1) / 2;
            Point memory _point = IVQst(VQst).userPointHistory(_user, _mid);
            if (_point.timestamp <= _timestamp) {
                _min = _mid;
            } else {
                _max = _mid - 1;
