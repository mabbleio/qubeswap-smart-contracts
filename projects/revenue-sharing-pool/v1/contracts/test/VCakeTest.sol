...
/// This contract was used to check whether the formula for calculating the locked amount in the pool is correct.
contract VQstTest {
    ...
    modifier onlyQstPool() {
        require(msg.sender == address(QstPool), "Not qst pool");
        _;
    }
     ...
     * @param _pid: qst pool ID in MasterChefV2
     */
    constructor(
        IQstPool _qstPool,
    ...
    /// @notice Return the balance of VQst at a given "_blockNumber"
    /// @param _user The address to get a balance of VQst
    /// @param _blockNumber The speicific block number that you want to check the balance of VQst
    ...
    function _checkpoint(
        ...
        uint256 MAX_LOCK = QstPool.MAX_LOCK_DURATION();
    ...
    function syncFromQstPool() external onlyNotInitialized {
        initialization[msg.sender] = true;
        (, , , , , uint256 lockEndTime, , , uint256 lockedAmount) = QstPool.userInfo(msg.sender);
        ...
        if (lockedAmount > 0) {
            ...
            prevLocks[msg.sender] = LockedBalance({amount: 0, end: 0});
            locks[msg.sender] = LockedBalance({amount: newLocked.amount, end: newLocked.end});
    ...
    struct DepositCache {
        uint256 shares;
        uint256 qstAtLastUserAction;
        uint256 lockStartTime;
        uint256 lockEndTime;
        uint256 userBoostedShare;
        bool locked;
        uint256 lockedAmount;
        uint256 qstPoolAvaliable;
    }
    ...
    function deposit(
        ...
        if (initialization[_user]) {
            ...
            (
                ...
                cache.locked,
                cache.lockedAmount
            ) = QstPool.userInfo(_user);
            // will check whether need to process the endTime later
            // cache.lockEndTime = _timestampToFloorWeek(cache.lockEndTime);
            ...
            console.log(_user, _amount, _lockDuration);
            console.log("prevLocked", userPrevLockedAmount[_user], cache.lockEndTime);
            // if (cache.shares == 0) {
            //     // user try to lock with clean address
            //     newLocked.amount = _amount;
            //     newLocked.end = _lockDuration + block.timestamp;
            // } else
            ...
            uint256 totalBoostDebt = QstPool.totalBoostDebt();
            cache.qstPoolAvaliable = QstPool.available();

            // need to calculate pendding qst when harvest
            uint256 pendingQst = MasterchefV2.pendingQst(QstPoolPID, address(QstPool));
            if (pendingQst > 0) {
                cache.qstPoolAvaliable += pendingQst;
            }

            // simulate QstPool.updateUserShare() function
            if (cache.shares > 0) {
                if (cache.locked) {
                    // Calculate the user's current token amount and update related parameters.
                    uint256 currentAmount = ((cache.qstPoolAvaliable + totalBoostDebt) * (cache.shares)) /
                        totalShares -
                        cache.userBoostedShare;
                    ...
                    if (
                        !QstPool.freeOverdueFeeUsers(_user) &&
                        ...
                        uint256 overdueWeight = (overdueDuration * QstPool.overdueFee()) / DURATION_FACTOR_OVERDUE;
                        uint256 currentOverdueFee = (earnAmount * overdueWeight) / PRECISION_FACTOR;
                        // token.safeTransfer(treasury, currentOverdueFee); need to reduce qst balance in qst pool
                        cache.qstPoolAvaliable -= currentOverdueFee;

                        currentAmount -= currentOverdueFee;

                        console.log("Charge overdue fee", currentOverdueFee);
                    }
                    // Recalculate the user's share.
                    // uint256 pool = QstPool.balanceOf();
                    uint256 currentShares;
                    if (totalShares != 0) {
                        currentShares =
                            (currentAmount * totalShares) /
                            ((cache.qstPoolAvaliable + totalBoostDebt) - currentAmount);
                    } else {
                    ...
                    if (cache.lockEndTime < block.timestamp) {
                        ...
                        cache.lockEndTime = 0;
                        // totalLockedAmount -= cache.lockedAmount;
                        cache.lockedAmount = 0;
                    }
                } else if (!QstPool.freePerformanceFeeUsers(_user)) {
                    // Calculate Performance fee.
                    uint256 totalAmount = (cache.shares * (cache.qstPoolAvaliable + totalBoostDebt)) / totalShares;
                    totalShares -= cache.shares;
                    cache.shares = 0;
                    uint256 earnAmount = totalAmount - cache.qstAtLastUserAction;
                    uint256 feeRate = QstPool.performanceFee();
                    if (_isContract(_user)) {
                        feeRate = QstPool.performanceFeeContract();
                    }
                    uint256 currentPerformanceFee = (earnAmount * feeRate) / 10000;
                    if (currentPerformanceFee > 0) {
                        // token.safeTransfer(treasury, currentPerformanceFee); need to reduce qst balance in qst pool
                        cache.qstPoolAvaliable -= currentPerformanceFee;
                    ...
                    if (totalShares != 0) {
                        newShares =
                            ...
                            ((cache.qstPoolAvaliable + totalBoostDebt) - totalAmount);
                    } else {
            ...
            // uint256 userCurrentShares;
            // uint256 userCurrentAmount;
            uint256 userCurrentLockedBalance;
            // uint256 pool = QstPool.balanceOf();
            // if (_amount > 0) {
            //     // token.safeTransferFrom(_user, address(this), _amount);
            //     // cache.qstPoolAvaliable += _amount;
            //     userCurrentAmount = _amount;
            // }

            // Calculate lock funds
            if (cache.shares > 0 && cache.locked) {
                userCurrentLockedBalance = ((cache.qstPoolAvaliable + totalBoostDebt) * cache.shares) / totalShares;
                // userCurrentAmount += userCurrentLockedBalance;
                // totalShares -= cache.shares;
                // cache.shares = 0;

                // Update lock amount
                if (cache.lockStartTime == block.timestamp) {
                    cache.lockedAmount = userCurrentLockedBalance;
                    // totalLockedAmount += user.cache.lockedAmount;
                }
            }
            // if (totalShares != 0) {
            //     userCurrentShares =
            //         (userCurrentAmount * totalShares) /
            //         ((cache.qstPoolAvaliable + totalBoostDebt) - userCurrentLockedBalance);
            // } else {
            //     userCurrentShares = userCurrentAmount;
            // }

            // Calculate the boost weight share.
            if (cache.lockEndTime > cache.lockStartTime) {
                // Calculate boost share.
                // uint256 boostWeight = ((cache.lockEndTime - cache.lockStartTime) * QstPool.BOOST_WEIGHT()) /
                //     DURATION_FACTOR;
                // uint256 boostShares = (boostWeight * userCurrentShares) / PRECISION_FACTOR;
                // userCurrentShares += boostShares;
                // cache.shares += userCurrentShares;

                // // Calculate boost share , the user only enjoys the reward, so the principal needs to be recorded as a debt.
                // uint256 boostedShare = (boostWeight * userCurrentAmount) / PRECISION_FACTOR;
                // cache.userBoostedShare += boostedShare;
                // totalBoostDebt += boostedShare;
            ...
            // }
            // totalShares += userCurrentShares;

            // user.cache.qstAtLastUserAction = (user.cache.shares * balanceOf()) / totalShares - user.cache.userBoostedShare;
            ...
            prevLocks[_user] = LockedBalance({amount: prevLocked.amount, end: prevLocked.end});
            locks[_user] = LockedBalance({amount: newLocked.amount, end: newLocked.end});
    ...
    function withdraw(address _user) external onlyQstPool {
        if (initialization[_user]) {
            (, , , , , uint256 lockEndTime, , , uint256 lockedAmount) = QstPool.userInfo(_user);
            LockedBalance memory prevLocked = LockedBalance({
                amount: SafeCast.toInt128(int256(lockedAmount)),
                end: _timestampToFloorWeek(lockEndTime)
            });
    ...
    /// @notice Calculate total supply of VQst (voting power)
    function totalSupply() external view returns (uint256) {
        return _totalSupplyAt(pointHistory[epoch], block.timestamp);
    }

    /// @notice Calculate total supply of VQst at specific block
    ...
    /// @notice Calculate total supply of VQst (voting power) at some point in the past
