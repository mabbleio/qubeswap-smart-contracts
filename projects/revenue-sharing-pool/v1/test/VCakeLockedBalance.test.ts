...
import VQstTestArtifact from "./artifactsFile/VQstTest.json";

const ZERO = BigNumber.from(0);
const DAY = BigNumber.from(86400);
const WEEK = DAY.mul(7);

describe("VQst LockedBalance", () => {
  let QstPoolSC;
  let VQstTestSC;
  let QubeSwapTokenSC;
  let admin;
  ...
  beforeEach(async () => {
    ...
    // await QubeSwapTokenSC["mint(address,uint256)"](user2, ethers.utils.parseUnits("100000000"));
    // await QubeSwapTokenSC["mint(address,uint256)"](user3, ethers.utils.parseUnits("100000000"));
    ...
    const VQstTest = await ethers.getContractFactoryFromArtifact(VQstTestArtifact);
    VQstTestSC = await VQstTest.deploy(QstPoolSC.address, masterChefV2.address, 2);

    await QstPoolSC.setVQstContract(VQstTestSC.address);
  });
  ...
  describe("Check whether the formula for calculating the locked amount in the pool is correct when executing syncFromQstPool at first", () => {
    beforeEach(async function () {
      await VQstTestSC.connect(user1).syncFromQstPool();
    });

    it("Deposit flexible for the first time in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 0);
      await time.advanceBlockTo((await time.latestBlock()).toNumber() + 100);
      const user1Info = await QstPoolSC.userInfo(user1.address);

      const prevLockedBalance = await VQstTestSC.prevLocks(user1.address);
      const currentLockedBalance = await VQstTestSC.locks(user1.address);
    ...
    it("Lock for the first time in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
    ...
    it("Extend lock duration for locked account in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      const currentTime = (await time.latest()).toNumber();

      await time.advanceBlockTo((await time.latestBlock()).toNumber() + 100);

      const prevUser1Info = await QstPoolSC.userInfo(user1.address);

      expect(prevUser1Info.lockEndTime).to.deep.eq(BigNumber.from(currentTime + 3600 * 24 * 30));

      await QstPoolSC.connect(user1).deposit(0, 3600 * 24 * 30);

      const currentUser1Info = await QstPoolSC.userInfo(user1.address);
    ...
    it("Deposit more qst for locked account in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
    ...
    it("Deposit more qst and extend duration for locked account in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      ...
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 60);
    ...
    it("Lock again after expired in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
    ...
    it("Unlock for locked account without overdue fee in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      ...
      await QstPoolSC.connect(user1).unlock(user1.address);
    ...
    it("Unlock for locked account with overdue fee in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
    ...
    it("Withdraw part in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      ...
      await QstPoolSC.connect(user1).withdraw(prevUser1Info.shares.div(BigNumber.from(2)));
    ...
    it("Withdraw all in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      ...
      await QstPoolSC.connect(user1).withdraw(prevUser1Info.shares);
    ...
    it("Lock again after Withdraw part in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);

      let prevUser1Info = await QstPoolSC.userInfo(user1.address);

      await time.increase(3600 * 24 * 32);

      await QstPoolSC.connect(user1).withdraw(prevUser1Info.shares.div(BigNumber.from(2)));

      prevUser1Info = await QstPoolSC.userInfo(user1.address);
    ...
    it("Lock again with overdue fee before DURATION_FACTOR_OVERDUE in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
    ...
    it("Lock again with overdue fee after DURATION_FACTOR_OVERDUE in QstPool", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
  ...
  describe("Check whether the formula for calculating the locked amount in the pool is correct when executing syncFromQstPool at any time", () => {
    beforeEach(async function () {});

    it("Lock for the first time in QstPool , executing syncFromQstPool after this", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      await VQstTestSC.connect(user1).syncFromQstPool();
    ...
    it("Extend lock duration for locked account in QstPool, executing syncFromQstPool after the first lock", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      await VQstTestSC.connect(user1).syncFromQstPool();
      await time.advanceBlockTo((await time.latestBlock()).toNumber() + 100);
    ...
    it("Deposit more qst for locked account in QstPool, executing syncFromQstPool after the first lock", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      await VQstTestSC.connect(user1).syncFromQstPool();
    ...
    it("Deposit more qst and extend duration for locked account in QstPool, executing syncFromQstPool after the first lock", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
      await VQstTestSC.connect(user1).syncFromQstPool();
    ...
    it("Lock again after expired in QstPool,executing syncFromQstPool after expired", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
    ...
    it("Unlock for locked account without overdue fee in QstPool, executing syncFromQstPool after expired", async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 30);
