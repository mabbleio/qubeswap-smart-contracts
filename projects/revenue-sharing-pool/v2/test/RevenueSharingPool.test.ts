...
describe("Revenue Sharing Pool", () => {
  ...
  beforeEach(async () => {
    ...
    RevenueSharingPoolFactorySC = await RevenueSharingPoolFactory.deploy(VEQstSC.address);
  ...
  describe("initialized", async () => {
    it("should initialized correctly", async () => {
      ...
      expect(await RevenueSharingPoolSC.VEQst()).to.deep.eq(VEQstSC.address);
  ...
  describe("checkpointToken", () => {
    beforeEach(async function () {
      ...
      // update qst pool
      await QstPoolSC.connect(user4).deposit(ethers.utils.parseUnits("1"), 0);

      await VEQstSC.connect(user1).migrateFromQstPool();
      await VEQstSC.connect(user2).migrateFromQstPool();
      await VEQstSC.connect(user3).migrateFromQstPool();
    });
  ...
  describe("totalSupplyAt and balanceOfAt", () => {
    ...
    context("One user", async () => {
      it("The user's balanceOfAt is equal to the totalSupplyAt at specific week cursor when have only one user", async () => {
        ...
        let userinfoInVEQst = await VEQstSC.getUserInfo(user1.address);
        ...
        totalSupplyAt = await RevenueSharingPoolSC.totalSupplyAt(nextWeekCursor);
        user1BalanceOfAt = await RevenueSharingPoolSC.balanceOfAt(user1.address, nextWeekCursor);
        const totalSupplyAtInVEQst = await VEQstSC.totalSupplyAt(weekBlock.toNumber());
        expect(user1BalanceOfAt).to.deep.eq(totalSupplyAt);
        expect(totalSupplyAtInVEQst).to.deep.eq(totalSupplyAt);
      });
    });
  ...
  describe("claim", () => {
    ...
    it("user claim 0 when lock after checkpointToken ", async () => {
      ...
      await QstPoolSC.connect(user4).deposit(ethers.utils.parseUnits("80000"), 3600 * 24 * 30);
      await VEQstSC.connect(user4).migrateFromQstPool();
  ...
  describe("Delegator claim", () => {
    ...
    beforeEach(async function () {
      ...
      await VEQstSC.connect(user1).delegateFromQstPool(delegatorSC.address);
      await VEQstSC.connect(user2).delegateFromQstPool(delegatorSC.address);
    ...
    it("The total balanceOfAt of all users is equal to the totalSupplyAt at specific week cursor", async () => {
      ...
      await VEQstSC.connect(user4).createLock(totalLockAmountOfDelegator, delegatorLockEndTime);
  ...
  describe("Inject Reward", () => {
    ...
    beforeEach(async function () {
      await QstPoolSC.connect(user1).deposit(ethers.utils.parseUnits("80000"), 3600 * 24 * 60);
      await QstPoolSC.connect(user2).deposit(ethers.utils.parseUnits("90000"), 3600 * 24 * 60);
      await QstPoolSC.connect(user3).deposit(ethers.utils.parseUnits("100000"), 3600 * 24 * 60);
    ...
    context("Can claim reward after inject reward timestamp", async () => {
      ...
      it("Using claimMany with qst pool proxy address , will get same reward in week four as used claim", async () => {
        await time.increaseTo(Week4.toNumber());

        await RevenueSharingPoolSC.checkpointToken();
        let userInfo = await VEQstSC.userInfo(user1.address);
        await RevenueSharingPoolSC.connect(user1).claimMany([userInfo.qstPoolProxy]);
        const user1Balance = await RevenueToken.balanceOf(user1.address);
        expect(user1Balance.gt(ZERO)).to.deep.eq(true);
        expect(user1Balance).to.deep.eq(rewardInWeekFour);
      });
      ...
      it("Using claim with qst pool proxy address , will get same reward in week four as used claim", async () => {
        await time.increaseTo(Week4.toNumber());

        await RevenueSharingPoolSC.checkpointToken();
        let userInfo = await VEQstSC.userInfo(user1.address);
        await RevenueSharingPoolSC.connect(user1).claim(userInfo.qstPoolProxy);
        const user1Balance = await RevenueToken.balanceOf(user1.address);
        expect(user1Balance.gt(ZERO)).to.deep.eq(true);
        expect(user1Balance).to.deep.eq(rewardInWeekFour);
      });

      it("Using claimForUser with user address , will not get reward for qst pool proxy in week four as used claim", async () => {
        await time.increaseTo(Week4.toNumber());
      ...
      it("Using claimForUser with qst pool proxy address , will get same reward in week four as used claim", async () => {
        await time.increaseTo(Week4.toNumber());

        await RevenueSharingPoolSC.checkpointToken();
        let userInfo = await VEQstSC.userInfo(user1.address);
        await RevenueSharingPoolSC.connect(user1).claimForUser(userInfo.qstPoolProxy);
        const user1Balance = await RevenueToken.balanceOf(user1.address);
        expect(user1Balance.gt(ZERO)).to.deep.eq(true);
        expect(user1Balance).to.deep.eq(rewardInWeekFour);
      });
  ...
  describe("getRecipient", () => {
    ...
    it("Users proxy recipient will be user address when user had proxy and did not set recipient", async () => {
      let userInfo = await VEQstSC.userInfo(user1.address);
      let recipientAddress = await RevenueSharingPoolSC.getRecipient(userInfo.qstPoolProxy);
