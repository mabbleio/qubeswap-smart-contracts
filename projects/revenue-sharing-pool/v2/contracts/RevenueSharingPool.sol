...
contract RevenueSharingPool is Ownable, ReentrancyGuard {
    ...
    /// @dev VEQst supply at week bounds
    ...
    /// @notice Get VEQst balance of "_user" at "_timstamp"
    ...
    function balanceOfAt(address _user, uint256 _timestamp) external view returns (uint256) {
        (address qstPoolProxy, , , , , ) = IVEQst(VEQst).userInfo(_user);

        if (qstPoolProxy != address(0)) {
            return _balanceOfAt(_user, _timestamp) + _balanceOfAt(qstPoolProxy, _timestamp);
        } else {
    ...
    function balanceOfAtProxy(address _user, uint256 _timestamp) external view returns (uint256) {
        (address qstPoolProxy, , , , , ) = IVEQst(VEQst).userInfo(_user);
        if (qstPoolProxy != address(0)) {
            return _balanceOfAt(qstPoolProxy, _timestamp);
        }
    }
    ...
    function _balanceOfAt(address _user, uint256 _timestamp) internal view returns (uint256) {
        uint256 _maxUserEpoch = IVEQst(VEQst).userPointEpoch(_user);
        if (_maxUserEpoch == 0) {
            return 0;
        }

        uint256 _epoch = _findTimestampUserEpoch(_user, _timestamp, _maxUserEpoch);
        Point memory _point = IVEQst(VEQst).userPointHistory(_user, _epoch);
    ...
    function _checkpointTotalSupply() internal {
        ...
        for (uint256 _i = 0; _i < 52; _i++) {
            if (_weekCursor > _roundedTimestamp) {
                ...
                Point memory _point = IVEQst(VEQst).pointHistory(_epoch);
                int128 _timeDelta = 0;
    ...
    /// @notice Update VEQst total supply checkpoint
    ...
    function _claim(
        ...
        Point memory _userPoint = IVEQst(VEQst).userPointHistory(_user, _userEpoch);
        ...
        for (uint256 _i = 0; _i < 52; _i++) {
            ...
            if (_userWeekCursor >= _userPoint.timestamp && _userEpoch <= _maxUserEpoch) {
                ...
                if (_userEpoch > _maxUserEpoch) {
                    ...
                    _userPoint = IVEQst(VEQst).userPointHistory(_user, _userEpoch);
                }
            } else {
    ...
    function getRecipient(address _user) public view returns (address _recipient) {
        ...
        bool isQstPoolProxy = IVEQst(VEQst).isQstPoolProxy(_user);
        if (isQstPoolProxy) {
            _recipient = IProxyForQstPool(_user).qstPoolUser();
        }
    ...
    /// @notice Claim rewardToken for "_user", without qst pool proxy
    /// @param _user The address to claim rewards for
    ...
    /// @notice Claim rewardToken for user and user's qst pool proxy
    ...
    function claimTo(address _recipient) external nonReentrant onlyLive returns (uint256) {
        return _claimWithQstPoolProxy(msg.sender, _recipient);
    }
    ...
    function claim(address _user) external nonReentrant onlyLive returns (uint256) {
        return _claimWithQstPoolProxy(_user, address(0));
    }

    function _claimWithQstPoolProxy(address _user, address _recipient) internal returns (uint256) {
        if (block.timestamp >= weekCursor) _checkpointTotalSupply();
        ...
        (address qstPoolProxy, , , , , ) = IVEQst(VEQst).userInfo(_user);
        if (qstPoolProxy != address(0)) {
            _total += _claim(qstPoolProxy, _recipient, _lastTokenTimestamp);
        }
        if (_total != 0) {
    ...
    function claimMany(address[] calldata _users) external nonReentrant onlyLive returns (bool) {
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            (address qstPoolProxy, , , , , ) = IVEQst(VEQst).userInfo(_user);
            if (qstPoolProxy != address(0)) {
                _amount += _claim(qstPoolProxy, _recipient, _lastTokenTimestamp);
            }
            if (_amount != 0) {
                rewardToken.safeTransfer(_recipient, _amount);
    ...
    function _findTimestampEpoch(uint256 _timestamp) internal view returns (uint256) {
        uint256 _min = 0;
        uint256 _max = IVEQst(VEQst).epoch();
        ...
        for (uint256 i = 0; i < 128; i++) {
            ...
            Point memory _point = IVEQst(VEQst).pointHistory(_mid);
    ...
    function _findTimestampUserEpoch(
        ...
        for (uint256 i = 0; i < 128; i++) {
            ...
            uint256 _mid = (_min + _max + 1) / 2;
            Point memory _point = IVEQst(VEQst).userPointHistory(_user, _mid);
            if (_point.timestamp <= _timestamp) {
                _min = _mid;
            } else {
                _max = _mid - 1;
    ...
    /// @dev In case some partners integrated our VEQst , but did not write claim logic for revenue sharing pool. So owner can set recipient for them , partners still can get reward.
