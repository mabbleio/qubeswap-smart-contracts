...
contract RevenueSharingPoolGateway {
    /// @notice Claim multiple pools for user , with qst pool proxy
    /// @dev If user had qst pool proxy , will claim at the same time
    ...
    /// @notice Claim multiple pools for user, without qst pool proxy
    /// @dev If user had qst pool proxy , will not claim at the same time
    /// @dev You can use qst pool proxy address as _for , will claim for qst pool proxy
