...
contract("GaugeVoting Test", ([admin, user1, user2, user3, user4, user5, ...accounts]) => {
  let mockQst, mockVotingEscrow, voter;
  ...
  let QubePair1 = "0x0000000000000000000000000000000000000101";
  let QubePair2 = "0x0000000000000000000000000000000000000102";
  let QubePair3 = "0x0000000000000000000000000000000000000103";
  let QubePair4 = "0x0000000000000000000000000000000000000104";
  let QubePair5 = "0x0000000000000000000000000000000000000105";
  ...
  let veQstPool = "0x0000000000000000000000000000000000000301";
  ...
  let veQSTBscReceiver_Injector = "0x1000000000000000000000000000000000000007";
  ...
  describe("GaugeVoting #1 - USER VOTE", async () => {
    ...
    it("add types", async () => {
      ...
      result = await voter.addType("veQST Pool", 1, {
        from: admin,
      });
      expectEvent(result, "AddType", { name: "veQST Pool", type_id: "3" });
    });
    ...
    it("add gauges", async () => {
      ...
      // V3 LP on Bsc
      await voter.addGauge(QubePair1, 1, 0, 12, MasterChefV3BscReceiver, 56, 100, 0, {
        from: admin,
      });
      // Another V3 LP on Bsc
      await voter.addGauge(QubePair2, 1, 0, 13, MasterChefV3BscReceiver, 56, 100, 100, {
        from: admin,
      });
      // V3 LP on Arb
      await voter.addGauge(QubePair3, 1, 0, 14, MasterChefV3CrossChainReceiver, 42161, 100, 0, {
        from: admin,
      });
      // Another V3 LP on Arb
      await voter.addGauge(QubePair4, 1, 0, 15, MasterChefV3CrossChainReceiver, 42161, 100, 0, {
        from: admin,
      });
      // V3 LP on Eth
      await voter.addGauge(QubePair5, 1, 0, 0, MasterChefV3CrossChainReceiver, 1, 100, 0, {
        from: admin,
      });
      // ALM Wrapper on Bsc
      ...
      // veQST Pool
      await voter.addGauge(veQstPool, 3, 0, 0, veQSTBscReceiver_Injector, 56, 100, 500, {
        from: admin,
      });
    ...
    it("gauge1 start vote", async () => {
      ...
      await voter.voteForGaugeWeightsBulk(
        [V2Staker1, V2Staker2, QubePair1, QubePair2, veQstPool],
    ...
    it("gauge3 start vote", async () => {
      // user2 vote
      await voter.voteForGaugeWeights(QubePair1, 1000, 56, false, false, {
        from: user2,
      });
      // user3 vote
      await voter.voteForGaugeWeights(QubePair1, 1000, 56, false, false, {
      ...
      let weight3 = await voter.getGaugeWeight(QubePair1, 56, false);
      assert.equal(formatUnits(weight3.toString()), String("2315.3110231014955296"));
    });

    it("gauge4 start vote", async () => {
      // user2 vote
      await voter.voteForGaugeWeights(QubePair2, 1000, 56, false, false, {
        from: user2,
      });
      // user3 vote
      await voter.voteForGaugeWeights(QubePair2, 1000, 56, false, false, {
      ...
      // // user4 vote
      // await voter.voteForGaugeWeights(QubePair2, 3000, 56, false, false, {
      //     from: user4
      // });

      let weight4 = await voter.getGaugeWeight(QubePair2, 56, false);
      assert.equal(formatUnits(weight4.toString()), String("6812.9187141853270944"));
    });

    it("gauge5 start vote", async () => {
      // user2 vote
      await voter.voteForGaugeWeights(QubePair3, 1000, 42161, false, false, {
        from: user2,
      });
      // user3 vote
      await voter.voteForGaugeWeights(QubePair3, 1000, 42161, false, false, {
      ...
      let weight5 = await voter.getGaugeWeight(QubePair3, 42161, false);
      assert.equal(formatUnits(weight5.toString()), String("66.5071775596081728"));
    });

    it("gauge6 start vote", async () => {
      // user2 vote
      await voter.voteForGaugeWeights(QubePair4, 1000, 42161, false, false, {
        from: user2,
      });
      // user3 vote
      await voter.voteForGaugeWeights(QubePair4, 1000, 42161, false, false, {
      ...
      let weight6 = await voter.getGaugeWeight(QubePair4, 42161, false);
      assert.equal(formatUnits(weight6.toString()), String("66.5071775596081728"));
    });

    it("gauge7 start vote", async () => {
      // user2 vote
      await voter.voteForGaugeWeights(QubePair5, 1000, 1, false, false, {
        from: user2,
      });
      // user3 vote
      await voter.voteForGaugeWeights(QubePair5, 1000, 1, false, false, {
      ...
      let weight7 = await voter.getGaugeWeight(QubePair5, 1, false);
    ...
    it("gauge12 start vote", async () => {
      // user3 vote
      await voter.voteForGaugeWeights(veQstPool, 1000, 56, false, false, {
        from: user3,
      });
      // user4 vote
      await voter.voteForGaugeWeights(veQstPool, 4000, 56, false, false, {
        from: user4,
      });

      let weight12 = await voter.getGaugeWeight(veQstPool, 56, false);
    ...
    //
    //     // admin vote
    //     await voter.voteFromAdminBulk([QubePair1, QubePair2, QubePair3, QubePair4, QubePair5], [800, 800, 800, 800, 800], [adminEndTime, adminEndTime, adminEndTime, adminEndTime, adminEndTime], [56, 56, 42161, 42161, 1], {
    //         from: admin
    //     });
    //
    //     // admin vote
    ...
    //
    //     // admin vote
    //     await voter.voteFromAdmin(veQstPool, 800, adminEndTime, 56, {
    //         from: admin
    //     });
    // });
    ...
    //     let weight3 = await voter.getGaugeWeight(QubePair1, 56, true);
    //     let weight4 = await voter.getGaugeWeight(QubePair2, 56, true);
    //     let weight5 = await voter.getGaugeWeight(QubePair3, 42161, true);
    //     let weight6 = await voter.getGaugeWeight(QubePair4, 42161, true);
    //     let weight7 = await voter.getGaugeWeight(QubePair5, 1, true);
    ...
    //     let weight12 = await voter.getGaugeWeight(veQstPool, 56, true);
    ...
    // //     await voter.updateGaugeInfo(QubePair2, 13, MasterChefV3BscReceiver, 56, 100, 0, {
    // //         from: admin
    // //     });
    // //     let weight4 = await voter.getGaugeWeight(QubePair2, 56, true);
    // //     assert.equal(String(weight4), String("7106151799211914512000"));
    // //
    // //     await voter.updateGaugeInfo(QubePair2, 13, MasterChefV3BscReceiver, 56, 100, 100, {
    // //         from: admin
    // //     });
    // //     let weight4_new = await voter.getGaugeWeight(QubePair2, 56, true);
