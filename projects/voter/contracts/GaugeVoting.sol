...
contract GaugeVoting is Ownable, Pausable {
    ...
    function voteForGaugeWeights(
        ...
        (address qstPoolProxy, , , , , ) = VotingEscrow(escrow).userInfo(msg.sender);

        if (!_skipProxy && qstPoolProxy != address(0)) {
            (int128 amount1, uint256 lockEnd1) = VotingEscrow(escrow).locks(qstPoolProxy);
            uint256 slope1Uint256 = SafeCast.toUint256(amount1) / MAX_LOCK_TIME;
            if (
                slope1Uint256 > 0 &&
                lockEnd1 > nextTime &&
                block.timestamp >= lastUserVote[qstPoolProxy][gauge_hash] + WEIGHT_VOTE_DELAY
            ) {
                _voteFromUser(gauge_hash, qstPoolProxy, _user_weight, slope1Uint256, lockEnd1);
            }
        }
    }
