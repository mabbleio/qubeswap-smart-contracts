...
import "./interfaces/IQubeV3Pool.sol";
import "./interfaces/INonfungiblePositionManager.sol";
import "./interfaces/IQubeV3Factory.sol";

contract FarmBooster is Ownable {
    using IterableMapping for ItMap;

    /// @notice VEQst.
    address public immutable VEQst;
    /// @notice VEQst caller, this smart contract will trigger depositFor and unlock.
    address public VEQstCaller;
    ...
    /// @notice QubeV3Factory contract.
    IQubeV3Factory public immutable QubeV3Factory;
    ...
    event UpdateVEQstCaller(address VEQstCaller);

    /// @param _VEQst VEQst contract address.
    ...
    constructor(
        ...
        QubeV3Factory = IQubeV3Factory(NonfungiblePositionManager.factory());
    }
    ...
    /// @notice Checks if the msg.sender is the veqst caller.
    modifier onlyVEQstCaller() {
        require(msg.sender == VEQstCaller, "Not veqst caller");
        _;
    }

    /// @notice set VEQst caller.
    /// @param _VEQstCaller VEQst caller.
    function setVEQstCaller(address _VEQstCaller) external onlyOwner {
        VEQstCaller = _VEQstCaller;
        emit UpdateVEQstCaller(_VEQstCaller);
    }
    ...
    /// @notice VEQst operation(deposit/withdraw) automatically call this function.
    ...
    function depositFor(
        ...
        uint256 _actionType,
        bool _isQstPoolUser
    ) external onlyVEQstCaller {
        _updateUserAllBoostMultiplier(_for);
    }

    /// @notice Function to perform withdraw and unlock Qst for a user
    ...
    /// @param _withdrawAmount Qst amount
    function unlock(
        address _user,
        int128 _prevLockedAmount,
        uint256 _prevLockedEnd,
    ...
    function getUserMultiplier(uint256 _tokenId) external view returns (uint256) {
        ...
        if (pid == 0) {
            ...
            address v3PoolAddress = QubeV3Factory.getPool(token0, token1, fee);
    ...
    function _getTotalLiquidity(
        ...
        uint256 v3PoolLiquidity = IQubeV3Pool(v3Pool).liquidity();
    ...
    function _boostCalculate(
        ...
        uint256 totalSupplyInVEQst = IVEQst(VEQst).totalSupply();
        if (totalSupplyInVEQst == 0) return BOOST_PRECISION;
        uint256 aB = (totalLiquidity * IVEQst(VEQst).balanceOf(_user) * realCB) / totalSupplyInVEQst / CB_PRECISION;
