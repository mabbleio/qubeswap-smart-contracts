...
const QubePredictionV3 = artifacts.require("QubePredictionV3");
...
const DECIMALS = 8; // Chainlink default for QST/USD
...
const MIN_BET_AMOUNT = ether("1"); // 1 QST
...
contract(
  ...
  ([operator, admin, owner, bullUser1, bullUser2, bullUser3, bearUser1, bearUser2, bearUser3]) => {
    ...
    beforeEach(async () => {
      // Deploy MockQst
      mockQst = await MockERC20.new("Mock QST", "QST", _totalInitSupply);
      // mint qst for test accounts
      const MintAmount = ether("100"); // 100 qst
      mockQst.mintTokens(MintAmount, { from: bullUser1 });
      mockQst.mintTokens(MintAmount, { from: bullUser2 });
      mockQst.mintTokens(MintAmount, { from: bullUser3 });
      mockQst.mintTokens(MintAmount, { from: bearUser1 });
      mockQst.mintTokens(MintAmount, { from: bearUser2 });
      mockQst.mintTokens(MintAmount, { from: bearUser3 });

      oracle = await Oracle.new(DECIMALS, INITIAL_PRICE);

      prediction = await QubePredictionV3.new(
        mockQst.address,
        oracle.address,
        admin,
        operator,
      ...
      // approve qst amount for prediction contract
      const ApproveAmount = ether("10000000000000");
      mockQst.approve(prediction.address, ApproveAmount, { from: bullUser1 });
      mockQst.approve(prediction.address, ApproveAmount, { from: bullUser2 });
      mockQst.approve(prediction.address, ApproveAmount, { from: bullUser3 });
      mockQst.approve(prediction.address, ApproveAmount, { from: bearUser1 });
      mockQst.approve(prediction.address, ApproveAmount, { from: bearUser2 });
      mockQst.approve(prediction.address, ApproveAmount, { from: bearUser3 });
    });

    it("Initialize", async () => {
      assert.equal(await mockQst.balanceOf(prediction.address), 0);
    ...
    it("Should record data and user bets", async () => {
      ...
      await prediction.betBull(currentEpoch, ether("1.1"), { from: bullUser1 }); // 1.1 QST
      await prediction.betBull(currentEpoch, ether("1.2"), { from: bullUser2 }); // 1.2 QST
      await prediction.betBear(currentEpoch, ether("1.4"), { from: bearUser1 }); // 1.4 QST

      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), ether("3.7").toString()); // 3.7 QST
      assert.equal((await prediction.rounds(1)).totalAmount, ether("3.7").toString()); // 3.7 QST
      assert.equal((await prediction.rounds(1)).bullAmount, ether("2.3").toString()); // 2.3 QST
      assert.equal((await prediction.rounds(1)).bearAmount, ether("1.4").toString()); // 1.4 QST
      ...
      await prediction.betBull(currentEpoch, ether("2.1"), { from: bullUser1 }); // 2.1 QST
      await prediction.betBull(currentEpoch, ether("2.2"), { from: bullUser2 }); // 2.2 QST
      await prediction.betBear(currentEpoch, ether("2.4"), { from: bearUser1 }); // 2.4 QST

      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), ether("10.4").toString()); // 10.4 QST (3.7+6.7)
      assert.equal((await prediction.rounds(2)).totalAmount, ether("6.7").toString()); // 6.7 QST
      assert.equal((await prediction.rounds(2)).bullAmount, ether("4.3").toString()); // 4.3 QST
      assert.equal((await prediction.rounds(2)).bearAmount, ether("2.4").toString()); // 2.4 QST
      ...
      await prediction.betBull(currentEpoch, ether("3.1"), { from: bullUser1 }); // 3.1 QST
      await prediction.betBull(currentEpoch, ether("3.2"), { from: bullUser2 }); // 3.2 QST
      await prediction.betBear(currentEpoch, ether("3.4"), { from: bearUser1 }); // 3.4 QST

      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), ether("20.1").toString()); // 20.1 QST (3.7+6.7+9.7)
      assert.equal((await prediction.rounds(3)).totalAmount, ether("9.7").toString()); // 9.7 QST
      assert.equal((await prediction.rounds(3)).bullAmount, ether("6.3").toString()); // 6.3 QST
      assert.equal((await prediction.rounds(3)).bearAmount, ether("3.4").toString()); // 3.4 QST
      ...
      await prediction.betBull(currentEpoch, ether("4.1"), { from: bullUser1 }); // 4.1 QST
      await prediction.betBull(currentEpoch, ether("4.2"), { from: bullUser2 }); // 4.2 QST
      await prediction.betBear(currentEpoch, ether("4.4"), { from: bearUser1 }); // 4.4 QST

      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), ether("32.8").toString()); // 32.8 QST (3.7+6.7+9.7+12.7)
      assert.equal((await prediction.rounds(4)).totalAmount, ether("12.7").toString()); // 12.7 QST
      assert.equal((await prediction.rounds(4)).bullAmount, ether("8.3").toString()); // 8.3 QST
      assert.equal((await prediction.rounds(4)).bearAmount, ether("4.4").toString()); // 4.4 QST
    ...
    it("Should record rewards", async () => {
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), ether("3.7").toString());
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal(
        (await mockQst.balanceOf(prediction.address)).toString(),
      ...
      assert.equal((await prediction.rounds(1)).rewardBaseCalAmount, ether("2.3").toString()); // 2.3 QST, Bull total
      assert.equal((await prediction.rounds(1)).rewardAmount, ether("3.7") * INITIAL_REWARD_RATE); // 3.33 QST, Total * rewardRate
      assert.equal((await prediction.rounds(2)).rewardBaseCalAmount, 0);
      assert.equal((await prediction.rounds(2)).rewardAmount, 0);
      assert.equal(await prediction.treasuryAmount(), ether("3.7") * INITIAL_TREASURY_RATE); // 3.7 QST, Total * treasuryRate
      assert.equal(
        (await mockQst.balanceOf(prediction.address)).toString(),
      ...
      assert.equal((await prediction.rounds(1)).rewardBaseCalAmount, ether("2.3").toString()); // 2.3 QST, Bull total
      assert.equal((await prediction.rounds(1)).rewardAmount, ether("3.7") * INITIAL_REWARD_RATE); // 3.33 QST, Total * rewardRate
      assert.equal((await prediction.rounds(2)).rewardBaseCalAmount, ether("2.4").toString()); // 2.4 QST, Bear total
      assert.equal((await prediction.rounds(2)).rewardAmount, ether("6.7") * INITIAL_REWARD_RATE); // 6.7 QST, Total * rewardRate
    ...
    it("Should claim rewards", async () => {
      ...
      await prediction.betBull(currentEpoch, ether("1"), { from: bullUser1 }); // 1 QST
      await prediction.betBull(currentEpoch, ether("2"), { from: bullUser2 }); // 2 QST
      await prediction.betBear(currentEpoch, ether("4"), { from: bearUser1 }); // 4 QST
      ...
      await prediction.betBull(currentEpoch, ether("21"), { from: bullUser1 }); // 21 QST
      await prediction.betBull(currentEpoch, ether("22"), { from: bullUser2 }); // 22 QST
      await prediction.betBear(currentEpoch, ether("24"), { from: bearUser1 }); // 24 QST
    ...
    it("Should multi claim rewards", async () => {
      ...
      // 4.2 = 2/3 * (7*0.9) + // 30.851162790697674418 = 22 / 43 * (67 * 0.9) = 35.051162790697674418 QST
    ...
    it("Should claim treasury rewards", async () => {
      let predictionCurrentQST = ether("0");
      assert.equal(await mockQst.balanceOf(prediction.address), 0);
      ...
      await prediction.betBull(currentEpoch, ether("2"), { from: bullUser2 }); // 2 QST
      await prediction.betBear(currentEpoch, ether("4"), { from: bearUser1 }); // 4 QST
      predictionCurrentQST = predictionCurrentQST.add(ether("7"));

      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), predictionCurrentQST.toString());
      ...
      await prediction.betBull(currentEpoch, ether("22"), { from: bullUser2 }); // 22 QST
      await prediction.betBear(currentEpoch, ether("24"), { from: bearUser1 }); // 24 QST
      predictionCurrentQST = predictionCurrentQST.add(ether("67"));
      ...
      await prediction.betBull(currentEpoch, ether("31"), { from: bullUser1 }); // 31 QST
      await prediction.betBull(currentEpoch, ether("32"), { from: bullUser2 }); // 32 QST
      await prediction.betBear(currentEpoch, ether("34"), { from: bearUser1 }); // 34 QST
      predictionCurrentQST = predictionCurrentQST.add(ether("97"));
      ...
      predictionCurrentQST = predictionCurrentQST.sub(ether("0.7"));
      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), predictionCurrentQST.toString());
      ...
      assert.equal(await prediction.treasuryAmount(), 0); // Empty
      predictionCurrentQST = predictionCurrentQST.sub(ether("16.4"));
      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), predictionCurrentQST.toString());
    });
    ...
    it("Should refund rewards", async () => {
      ...
      assert.equal(await prediction.treasuryAmount(), 0);
      assert.equal((await mockQst.balanceOf(prediction.address)).toString(), ether("7").toString());
    ...
    it("recoverToken function work as expected", async () => {
      ...
      await prediction.betBull(currentEpoch, ether("1"), { from: bullUser2 });
      await prediction.betBear(currentEpoch, ether("1"), { from: bearUser1 });
      assert.equal(await mockQst.balanceOf(prediction.address), ether("3").toString());
      await expectRevert(
        prediction.recoverToken(mockQst.address, ether("1"), { from: owner }),
