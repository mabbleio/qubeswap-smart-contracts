...
contract VEQst is Ownable, ReentrancyGuard {
    ...
    event MigrateFromQstPool(address indexed user, address indexed proxy, uint256 amount, uint256 endTime);
    event DelegateFromQstPool(address indexed user, address indexed delegator, uint256 amount, uint256 endTime);
    ...
    // When user delegated their locked qst to delegator from qst pool, the locked qst will permanently locked in the qst pool, which is equivalent to burn it.
    // And qube team will inject qst to the delegator from the burning part which was originally intended to be burned in the future.
    struct Delegator {
        uint104 delegatedQstAmount; // The total qst amount delegated from qst pool.
        uint104 notInjectedQstAmount; // The qst amount which qube have not injected to the delegator.
    ...
    // --- Constants ---
    uint16 public constant MIGRATION_FROM_QST_POOL_FLAG = 1;
    uint16 public constant DELEGATION_FROM_QST_POOL_FLAG = 2;
    ...
    // Token to be locked (Qst)
    IERC20 public immutable token;
    // Total supply of Qst that get locked
    uint256 public supply;

    IQstPool public immutable QstPool;

    IProxyForQstPoolFactory public immutable ProxyForQstPoolFactory;

    IFarmBooster public FarmBooster;

    // Qst pool migation initialization flag
    ...
    // Mapping (user => UserInfo) to keep qst pool related information for each user
    mapping(address => UserInfo) public userInfo;

    // Mapping (address => Delegator) to keep delegator related information
    mapping(address => Delegator) public delegator;

    // Mapping (user => Bool) to check whether this user is qst pool proxy smart contract
    mapping(address => bool) public isQstPoolProxy;
    ...
    // If you wish to build on top of VEQST, contact our BD team via: https://docs.qubeswap.finance/ecosystem-and-partnerships/business-partnerships for whitelisting
    ...
    modifier onlyQstPool() {
        require(msg.sender == address(QstPool), "! qst pool");
        _;
    }

    /**
     * @notice Constructor
     * @param _qstPool: Qst Pool contract
     * @param _token: Qst Token contract
     * @param _ProxyForQstPoolFactory The qst pool proxy factory
     */
    constructor(
        IQstPool _qstPool,
        IERC20 _token,
        IProxyForQstPoolFactory _ProxyForQstPoolFactory
    ) {
        QstPool = _qstPool;
        token = _token;
        ProxyForQstPoolFactory = _ProxyForQstPoolFactory;

        pointHistory.push(Point({bias: 0, slope: 0, timestamp: block.timestamp, blockNumber: block.number}));

        decimals = 18;

        name = "Vote-escrowed Qst";
        symbol = "veQst";
    }

    /// @notice Initialize for qst pool migration
    /// @dev Need to check whether qst pool conditions are met
    function initializeQstPoolMigration() external onlyOwner {
        require(!initialization, "Already initialized");
        address VQstInQstPool = QstPool.VQst();
        require(VQstInQstPool == address(this), "Bad VQst");
        initialization = true;
    }
    ...
    /// @return qstPoolProxy Proxy Smart Contract for users who had locked in qst pool
    /// @return qstAmount Qst amount locked in qst pool
    /// @return lockEndTime Record the lockEndTime in qst pool
    /// @return migrationTime Record the migration time
    /// @return qstPoolType 1: Migration, 2: Delegation
        ...
        )
    {
        ...
        qstPoolProxy = user.qstPoolProxy;
        qstAmount = user.qstAmount;
        lockEndTime = user.lockEndTime;
        migrationTime = user.migrationTime;
        qstPoolType = user.qstPoolType;
    ...
    function balanceOfAtForProxy(address _user, uint256 _blockNumber) external view returns (uint256) {
        ...
        UserInfo memory user = userInfo[_user];
        if (user.qstPoolProxy != address(0)) {
            return _balanceOfAt(user.qstPoolProxy, _blockNumber);
        }
    }
    ...
    function balanceOfAt(address _user, uint256 _blockNumber) external view returns (uint256) {
        ...
        UserInfo memory user = userInfo[_user];
        if (user.qstPoolProxy != address(0)) {
            return _balanceOfAt(_user, _blockNumber) + _balanceOfAt(user.qstPoolProxy, _blockNumber);
        } else {
    ...
    function balanceOfForProxy(address _user) external view returns (uint256) {
        UserInfo memory user = userInfo[_user];
        if (user.qstPoolProxy != address(0)) {
            return _balanceOf(user.qstPoolProxy, block.timestamp);
        }
    }
    ...
    function balanceOf(address _user) external view returns (uint256) {
        UserInfo memory user = userInfo[_user];
        if (user.qstPoolProxy != address(0)) {
            return _balanceOf(_user, block.timestamp) + _balanceOf(user.qstPoolProxy, block.timestamp);
        } else {
        ...
        if (_address != address(0)) {
            ...
            if (_newLocked.end != 0) {
                ...
                if (_newLocked.end == _prevLocked.end) {
                    ...
                    // Possibly when user deposited more Qst to his locker
    ...
    /// @notice Deposit in qst pool
    /// @param _user user address
    /// @param _amount: number of tokens to deposit (in QST)
    /// @param _lockDuration: Token lock duration
    function deposit(
        address _user,
        uint256 _amount,
        uint256 _lockDuration
    ) external onlyQstPool {
        // Do not allow any user to deposit qst in qst pool now after migration initialized.
        // will forbid any deposit operatioin
        revert();
    }

    /// @notice Withdraw in qst pool
    /// @param _user user address
    function withdraw(address _user) external onlyQstPool {
        UserInfo memory user = userInfo[_user];

        // Users who had already delegated can not withdraw qst from qst pool
        if (user.qstPoolType == DELEGATION_FROM_QST_POOL_FLAG) {
            revert();
        } else if (
            user.qstPoolProxy != address(0) &&
            user.qstPoolType == MIGRATION_FROM_QST_POOL_FLAG &&
            user.withdrawFlag == NOT_WITHDRAW_FLAG
        ) {
            IProxyForQstPool(user.qstPoolProxy).withdrawAll();
            userInfo[_user].withdrawFlag = WITHDREW_FLAG;
        }
    }

    /// @notice Migrate from qst pool.
    function migrateFromQstPool() external nonReentrant {
        require(initialization, "! initialized");

        (uint256 shares, , , , , uint256 lockEndTime, uint256 userBoostedShare, , ) = QstPool.userInfo(msg.sender);

        require(lockEndTime > block.timestamp, "Lock expired");

        UserInfo storage user = userInfo[msg.sender];
        require(user.qstPoolType == 0, "Already migrated");

        user.qstPoolType = MIGRATION_FROM_QST_POOL_FLAG;
        uint256 totalShares = QstPool.totalShares();
        uint256 balanceOfQstPool = QstPool.balanceOf();
        // Subtract 1 is for precision round loss
        uint256 lockedQstAmount = (shares * balanceOfQstPool) / totalShares - userBoostedShare - 1;
        // will lock by proxy smart contract
        address proxy = ProxyForQstPoolFactory.deploy(msg.sender);
        isQstPoolProxy[proxy] = true;
        user.qstPoolProxy = proxy;
        user.migrationTime = uint48(block.timestamp);
        user.qstAmount = uint128(lockedQstAmount);
        user.lockEndTime = uint48(lockEndTime);

        IProxyForQstPool(proxy).createLockForProxy(lockedQstAmount, lockEndTime);

        emit MigrateFromQstPool(msg.sender, proxy, lockedQstAmount, lockEndTime);
    }

    /// @notice Delegate from qst pool.
    ...
    function delegateFromQstPool(address _delegator) external nonReentrant {
        require(initialization, "! initialized");
        ...
        user.qstPoolType = DELEGATION_FROM_QST_POOL_FLAG;
        ...
        require(lockedQstAmount > 0, "Bad _amount");
        require(_lock.amount > 0, "No lock on delegator");
        require(_lock.end > block.timestamp, "Delegator lock expired");

        _depositFor(_delegator, lockedQstAmount, 0, _lock, ACTION_DEPOSIT_FOR, true);

        delegatorInfo.delegatedQstAmount += uint104(lockedQstAmount);
        delegatorInfo.notInjectedQstAmount += uint104(lockedQstAmount);

        // Call delegate in delegator smart contract
        IDelegator(_delegator).delegate(msg.sender, lockedQstAmount, lockEndTime);
        emit DelegateFromQstPool(msg.sender, _delegator, lockedQstAmount, lockEndTime);
    }
    ...
    function migrationConvertToDelegation(address _delegator) external nonReentrant {
        ...
        require(user.qstPoolType == MIGRATION_FROM_QST_POOL_FLAG, "! migrated");
        require(user.withdrawFlag == NOT_WITHDRAW_FLAG, "Already withdraw");

        user.qstPoolType = DELEGATION_FROM_QST_POOL_FLAG;

        // Early withdraw all for proxy
        LockedBalance memory lockOfProxy = locks[user.qstPoolProxy];
        uint256 _amount = SafeCast.toUint256(lockOfProxy.amount);
        _unlock(user.qstPoolProxy, lockOfProxy, _amount);
        ...
        _depositFor(_delegator, user.qstAmount, 0, _lock, ACTION_DEPOSIT_FOR, true);

        delegatorInfo.delegatedQstAmount += uint104(user.qstAmount);
        delegatorInfo.notInjectedQstAmount += uint104(user.qstAmount);

        // Call delegate in delegator smart contract
        IDelegator(_delegator).delegate(msg.sender, user.qstAmount, user.lockEndTime);
        emit DelegateFromQstPool(msg.sender, _delegator, user.qstAmount, user.lockEndTime);
        emit MigrationConvertToDelegation(msg.sender, _delegator, user.qstAmount, user.lockEndTime);
    }

    /// @notice Create a new lock.
    /// @dev This will crate a new lock and deposit Qst to VEQst Vault
    /// @param _amount the amount that user wishes to deposit
    /// @param _unlockTime the timestamp when Qst get unlocked, it will be
    ...
    function createLockForProxy(uint256 _amount, uint256 _unlockTime) external {
        require(isQstPoolProxy[msg.sender], "! proxy");
        _createLock(_amount, _unlockTime);
    }
    ...
    function _createLock(uint256 _amount, uint256 _unlockTime) internal {
        ...
        _depositFor(msg.sender, _amount, _unlockTime, _locked, ACTION_CREATE_LOCK, isQstPoolProxy[msg.sender]);
    }
    ...
    function depositFor(address _for, uint256 _amount) external nonReentrant {
        require(!isQstPoolProxy[_for], "Incorrect proxy");
    ...
    /// @notice Internal function to perform deposit and lock Qst for a user
    /// @param _for The address to be locked and received VEQst
    /// @param _amount The amount to deposit
    /// @param _unlockTime New time to unlock Qst. Pass 0 if no change.
    /// @param _prevLocked Existed locks[_for]
    /// @param _actionType The action that user did as this internal function shared among
    /// @param _isQstPoolUser This user is qst pool user or not
    ...
    function _depositFor(
        ...
        // Qst pool user do not need to transfer qst
        if (_amount != 0 && !_isQstPoolUser) {
    ...
    /// @param _amount The amount of Qst to be added to the lock
    ...
    /// @notice Calculate total supply of VEQst (voting power) at at specific timestamp
    ...
    /// @notice Calculate total supply of VEQst at specific block
    ...
    /// @notice Calculate total supply of VEQst (voting power) at some point in the past
    ...
    /// @notice Withdraw all Qst when lock has expired.
    /// @param _to The address which will receive the qst
    function withdrawAll(address _to) external nonReentrant {
        Delegator memory delegatorInfo = delegator[msg.sender];
        require(delegatorInfo.notInjectedQstAmount == 0, "Insufficient injection for delegator");
        ...
        // Qst pool proxy do not need to transfer qst.
        if (!isQstPoolProxy[msg.sender]) token.safeTransfer(_to, _amount);

        emit WithdrawAll(msg.sender, _to, _amount, block.timestamp);
    }

    /// @notice Early withdraw Qst with penalty.
    /// @param _to The address which will receive the qst
    /// @param _amount Qst amount
    function earlyWithdraw(address _to, uint256 _amount) external nonReentrant {
        ...
        if (delegatorInfo.isDelegator == DELEGATOR_FLAG) {
            ...
            require(lockedAmount - _amount >= delegatorInfo.notInjectedQstAmount, "Delegator balance exceeded");
        }
        ...
        if (!noPenaltyForEarlyWithdraw[msg.sender] && earlyWithdrawBpsPerWeek > 0) {
            ...
            // accumulate qst for redistribution
    ...
    /// @notice Inject qst for delegator
    /// @param _delegator The delegator address
    /// @param _amount Qst amount
    function injectToDelegator(address _delegator, uint256 _amount) external onlyOwner {
        Delegator storage delegatorInfo = delegator[_delegator];
        require(delegatorInfo.isDelegator == DELEGATOR_FLAG, "! delegator");
        if (_amount > delegatorInfo.notInjectedQstAmount) {
            _amount = delegatorInfo.notInjectedQstAmount;
        }
        delegatorInfo.notInjectedQstAmount -= uint104(_amount);
        token.safeTransferFrom(msg.sender, address(this), _amount);
